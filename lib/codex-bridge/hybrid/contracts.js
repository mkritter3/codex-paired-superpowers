...
export async function publishContract({
  ...
  await appendImplementerEventLocked(specPath, {
    ...
    runtime_kind: member.runtime_kind ?? member.adapter,
...
/**
 * Verify the post-integration shim realization (spec §7.1). After the backend branch is
 * integrated, the real backend contract file must export the shape the UI consumed for
 * `bodyHash`. The check is delegated to an injected `typecheck` (the only effectful step):
 * its `{ ok }` result decides whether the realized backend matches the consumed shim.
 * A failure is the `hybrid-contract-realization-mismatch` condition (the runner wraps the halt).
 *
 * @param {object} args
 * @param {string}   args.bodyHash               — the contract hash the UI shim was generated against.
 * @param {string[]} args.backendContractFiles   — the real backend contract source files (now integrated).
 * @param {(a:object)=>{ok:boolean,detail?:any}} [args.typecheck] — injected realized-shape verifier.
 * @returns {{ ok: boolean, detail?: any }}
 */
export function verifyShimRealization({ bodyHash, backendContractFiles, typecheck }) {
  if (!nonEmptyString(bodyHash)) {
    throw new HybridContractError('hybrid-contract-malformed', 'verifyShimRealization: bodyHash must be a non-empty string');
  }
  if (!Array.isArray(backendContractFiles) || backendContractFiles.length === 0) {
    throw new HybridContractError('hybrid-contract-malformed', 'verifyShimRealization: backendContractFiles must be a non-empty array');
  }
  if (typeof typecheck !== 'function') {
    // No verifier injected: the caller must supply one (real runner injects a tsc step).
    throw new HybridContractError('hybrid-contract-malformed', 'verifyShimRealization: a typecheck function must be injected');
  }
  const result = typecheck({ bodyHash, backendContractFiles });
  return { ok: Boolean(result && result.ok), detail: result?.detail };
}
