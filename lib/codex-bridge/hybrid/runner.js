...
import { classifyStatusFile } from '../dispatch-status.js';
...
export function classifyBackgroundStatus({ statusFile, taskAlive, runtimeMs, maxRuntimeMs }) {
  const classified = classifyStatusFile({
    statusFile,
    taskAlive,
    runtimeMs,
    maxRuntimeMs,
    halts: {
      timeout: 'hybrid-codex-background-timeout',
      lost: 'hybrid-codex-background-lost',
      failed: 'hybrid-codex-backend-failed',
    },
  });
  if (classified.state === 'blocked' || classified.state === 'needs-context') {
    return { state: 'failed', terminal: true, haltReason: 'hybrid-codex-backend-failed' };
  }
  const { state, terminal, haltReason } = classified;
  return { state, terminal, haltReason };
}
...
export async function hybridPreflight({
  ...
  // First effect: halt unsupported configured panels before ownership parsing,
  // transport checks, sidecar mutation, or either worktree.create call.
...
export async function runHybridSlice(args) {
  ...
  const dispatchInput = (owner, onLaunched) => ({
    ...
    ...(owner === backend ? { modelRole: 'implement', onLaunched } : {}),
  });
  ...
  const runOne = async (owner, fn) => {
    const startedPayload = { phase: 'dispatch-start', owner: owner.member_id };
    if (owner === backend) startedPayload.model_role = 'implement';
    await recordEvent(owner, 'started', startedPayload);
    let launchRecordPromise = null;
    const onLaunched = (snapshot) => {
      if (!launchRecordPromise) {
        launchRecordPromise = recordEvent(owner, 'checkpoint', { phase: 'launched', ...snapshot });
      }
      return launchRecordPromise;
    };
    try {
      const result = await fn(dispatchInput(owner, onLaunched));
      if (launchRecordPromise) await launchRecordPromise;
      const eventType = result.outcome === 'completed' ? 'completed' : result.outcome === 'cancelled' ? 'cancelled' : 'failed';
      await recordEvent(owner, eventType, {
