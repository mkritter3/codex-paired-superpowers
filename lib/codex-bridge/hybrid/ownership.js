...
/**
 * Required plan-level (logical) adapter per owner (spec §5). The runner maps the UI
 * owner's logical `claude-ui` to the actual runtime (`claude-inline` interactive /
 * `claude-subagent` autopilot); the backend owner uses the real `codex-background-bash`
 * transport. An owner declaring any other adapter is ambiguous ownership (spec §6).
 */
export const REQUIRED_OWNER_ADAPTER = Object.freeze({
  'claude-ui': 'claude-ui',
  'codex-backend': 'codex-background-bash',
});
...
export function validateHybridOwnership({ sliceFiles, implementers }) {
  ...
  for (const e of entries) {
    ...
    // Adapter/owner pairing (spec §5/§6): each owner must use its required adapter.
    const wantAdapter = REQUIRED_OWNER_ADAPTER[e.owner];
    if (e.adapter !== wantAdapter) {
      throw haltError(
        'hybrid-ownership-malformed',
        `owner "${e.owner}" (member "${e.member_id}") must use adapter "${wantAdapter}"; got ` +
          `"${e.adapter || '(none)'}"`
      );
    }
  }
