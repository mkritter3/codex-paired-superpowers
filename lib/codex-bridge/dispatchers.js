// v0.7.2 dispatcher registry loader.
//
...
//
// Per spec §6.5 (rev4), the registry lives at <plugin>/agents/dispatchers.json
// and is **transport-aware**: each entry's required fields depend on its
// `transport` value:
//   - transport: claude-subagent       → requires `agent` (subagent file name)
//   - transport: codex-background-bash → requires `contract` (docs path)
//
// The loader validates per-transport: agent file frontmatter for subagent
// transports; contract file existence for codex-background-bash. Drift throws
// at load time.
...
const ALLOWED_TRANSPORTS = ['claude-subagent', 'codex-background-bash'];
const REQUIRED_COMMON_KEYS = ['transport', 'tools', 'domains'];
const DEPRECATED_AGENT_NAMES = ['slice-implementer', 'slice-implementer-codex'];
// v0.8.0 — top-level registry keys that are NOT implementer entries. These are
// validated by their own consumers (e.g., expert-runtime reads `experts`); the
// implementer loader simply skips them so that adding new non-implementer
// sections does not regress getDispatcher/enforceDomainPolicy.
const RESERVED_NON_IMPLEMENTER_KEYS = new Set(['experts']);
...
function validateEntry(implementer, entry) {
  // Common fields required for every entry, regardless of transport.
  ...
  if (typeof entry.transport !== 'string' || !ALLOWED_TRANSPORTS.includes(entry.transport)) {
    throw new DispatcherRegistryError(
      'dispatcher-registry-malformed',
      `implementer "${implementer}" has invalid transport: "${entry.transport}"; expected one of ${ALLOWED_TRANSPORTS.join(', ')}`
    );
  }

  // Transport-specific required field + mutual-exclusion checks.
  if (entry.transport === 'claude-subagent') {
    if (typeof entry.agent !== 'string' || entry.agent.length === 0) {
      throw new DispatcherRegistryError(
        'dispatcher-registry-malformed',
        `implementer "${implementer}" with transport=claude-subagent must have a non-empty 'agent' field`
      );
    }
    if ('contract' in entry) {
      throw new DispatcherRegistryError(
        'dispatcher-registry-malformed',
        `implementer "${implementer}" with transport=claude-subagent must NOT have a 'contract' field`
      );
    }
    if (DEPRECATED_AGENT_NAMES.includes(entry.agent)) {
      throw new DispatcherRegistryError(
        'dispatcher-registry-malformed',
        `implementer "${implementer}" points at deprecated agent "${entry.agent}"; ${entry.agent === 'slice-implementer-codex' ? 'codex moved to docs/codex-implementer-contract.md in v0.7.2' : 'use a per-implementer agent file instead'}`
      );
    }
  } else if (entry.transport === 'codex-background-bash') {
    if (typeof entry.contract !== 'string' || entry.contract.length === 0) {
      throw new DispatcherRegistryError(
        'dispatcher-registry-malformed',
        `implementer "${implementer}" with transport=codex-background-bash must have a non-empty 'contract' field`
      );
    }
    if ('agent' in entry) {
      throw new DispatcherRegistryError(
        'dispatcher-registry-malformed',
        `implementer "${implementer}" with transport=codex-background-bash must NOT have an 'agent' field (codex contract lives in docs/, not agents/)`
      );
    }
  }
...
function validateAgentConsistency(implementer, entry, agentsDir) {
  // Only applies to claude-subagent transport. codex-background-bash entries
  // have no agent file to validate against; the contract path is checked by
  // validateContractConsistency.
  if (entry.transport !== 'claude-subagent') return;
...
export function loadRegistry(rootOverride) {
  ...
  const implementers = topLevelKeys.filter(k => !RESERVED_NON_IMPLEMENTER_KEYS.has(k));
  ...
  for (const impl of implementers) {
    ...
    validateContractConsistency(impl, parsed[impl], root);
  }
...
function validateContractConsistency(implementer, entry, root) {
  // Only applies to codex-background-bash transport. The contract path is
  // declared in the registry entry; we just verify the file exists at load
  // time. Drift (e.g., contract moved/renamed) throws here.
  if (entry.transport !== 'codex-background-bash') return;
  const contractPath = join(root, entry.contract);
  if (!existsSync(contractPath)) {
    throw new DispatcherRegistryError(
      'dispatcher-registry-malformed',
      `implementer "${implementer}" with transport=codex-background-bash references missing contract file: ${contractPath}`
    );
  }
}
...
export function getDispatcher(implementer, rootOverride) {
  ...
  if (RESERVED_NON_IMPLEMENTER_KEYS.has(implementer)) {
    const implKeys = Object.keys(registry).filter(k => !RESERVED_NON_IMPLEMENTER_KEYS.has(k));
    throw new DispatcherRegistryError(
      'implementer-directive-malformed',
      `unknown implementer: "${implementer}" (reserved registry key, not an implementer); registry has [${implKeys.join(', ')}]`
    );
  }
  ...
  if (!entry) {
    ...
    throw new DispatcherRegistryError(
      'implementer-directive-malformed',
      `unknown implementer: "${implementer}"; registry has [${implKeys.join(', ')}]`
    );
  }
  return entry;
}
