 ...
 * @param {{staleResponse: object, pendingPrompt: string, phase?: string, round?: number, role?: string, specPath?: string, planPath?: string, repoRoot?: string}} ctx
 * @param {{codexFn?: (args: {prompt: string, model?: string, config?: object}) => Promise<{threadId: string, content: any}>, openFn?: (args: {prompt: string, model?: string, cwd?: string}) => Promise<{threadId: string, content: any}>}} [deps]
...
export async function recoverStaleThread(specPath, ctx, deps = {}) {
  ...
  if (isAgy) {
    const threadConfig = recorded.legacy
      ? { role: recorded.role, cli: 'agy', ...resolveModelRoles({ repoRoot }).roles[recorded.role] }
      : { role: recorded.role, cli: 'agy', model: recorded.model, effort: recorded.effort };
