...
import { validateModelsBlock } from './models.js';
import { validateReviewPanelBlock } from './review-panel.js';

/** @typedef {Record<string, any>} ProjectConfig */
/** @typedef {{ok: false, error: {code: string, detail: string}}} ConfigErrorResult */
 ...
 *
 * @param {ProjectConfig} lv  live_verification object (mutated)
 */
 ...
 *
 * @param {ProjectConfig} cfg  parsed JSON object
 * @returns {null | ConfigErrorResult}
 */
 ...
 * Mutates `cfg`.
 *
 * @param {ProjectConfig} cfg  parsed JSON object (post-validation)
 */
function applyWorktreeBootstrapDefaults(cfg) {
  ...
  /** @type {string[]} */
  const symlinks = wb.symlinks;
  wb.symlinks = symlinks.map((path) => ({ path, required: true }));
}
...
function validate(cfg) {
  ...
  // ── mailbox (v0.7.3) ────────────────────────────────────────────────────────
  const mbErr = validateMailbox(cfg);
  if (mbErr) return mbErr;

  // ── models (v0.16.0, spec §1) ──────────────────────────────────────────────
  const modelsErr = validateModels(cfg);
  if (modelsErr) return modelsErr;

  // ── review_panel (v0.19.0, spec §1) ────────────────────────────────────────
  const reviewPanelError = validateReviewPanelBlock(cfg.review_panel);
  if (reviewPanelError) return { ok: false, error: reviewPanelError };
...
/** @param {ProjectConfig} cfg @returns {null | ConfigErrorResult} */
...
// v0.7.3 mailbox defaults — exported for tests + SKILL.md reference.
export const MAILBOX_DEFAULTS = Object.freeze({
  max_bytes: 262144,                    // 256 KB triggers archive rotation
  archive_policy: 'rotate',             // alternative: "drop"
  archive_retention_days: 30,
  archive_retention_count: 100,
});

const MAILBOX_ALLOWED_POLICIES = ['rotate', 'drop'];

/** @param {ProjectConfig} cfg @returns {null | ConfigErrorResult} */
function validateMailbox(cfg) {
  if (!('mailbox' in cfg) || cfg.mailbox === null || cfg.mailbox === undefined) {
    return null;
  }
  const mb = cfg.mailbox;
  if (typeof mb !== 'object' || Array.isArray(mb)) {
    return err('live-verification-config-malformed', 'mailbox must be an object');
  }
  if ('max_bytes' in mb) {
    const v = mb.max_bytes;
    if (typeof v !== 'number' || !Number.isFinite(v) || !Number.isInteger(v) || v <= 0) {
      return err(
        'live-verification-config-malformed',
        `mailbox.max_bytes must be a positive integer (bytes); got ${JSON.stringify(v)}`
      );
    }
  }
  if ('archive_policy' in mb) {
    if (!MAILBOX_ALLOWED_POLICIES.includes(mb.archive_policy)) {
      return err(
        'live-verification-config-malformed',
        `mailbox.archive_policy must be one of ${MAILBOX_ALLOWED_POLICIES.join(', ')}; got ${JSON.stringify(mb.archive_policy)}`
      );
    }
  }
  if ('archive_retention_days' in mb) {
    const v = mb.archive_retention_days;
    if (typeof v !== 'number' || !Number.isFinite(v) || !Number.isInteger(v) || v < 0) {
      return err(
        'live-verification-config-malformed',
        `mailbox.archive_retention_days must be a non-negative integer; got ${JSON.stringify(v)}`
      );
    }
  }
  if ('archive_retention_count' in mb) {
    const v = mb.archive_retention_count;
    if (typeof v !== 'number' || !Number.isFinite(v) || !Number.isInteger(v) || v < 0) {
      return err(
        'live-verification-config-malformed',
        `mailbox.archive_retention_count must be a non-negative integer; got ${JSON.stringify(v)}`
      );
    }
  }
  for (const k of Object.keys(mb)) {
    if (!['max_bytes', 'archive_policy', 'archive_retention_days', 'archive_retention_count'].includes(k)) {
      return err(
        'live-verification-config-malformed',
        `mailbox contains unknown key: ${k}`
      );
    }
  }
  return null;
...
/** @param {ProjectConfig} cfg @returns {null | ConfigErrorResult} */
function validateModels(cfg) {
  if (cfg.models === null || cfg.models === undefined) return null;
  const error = validateModelsBlock(cfg.models);
  return error ? { ok: false, error } : null;
}

/**
 * Apply mailbox defaults. Returns object with all keys filled.
 */
/** @param {ProjectConfig} cfg */
export function applyMailboxDefaults(cfg) {
  const explicit = (cfg && cfg.mailbox) || {};
  return {
    max_bytes: 'max_bytes' in explicit ? explicit.max_bytes : MAILBOX_DEFAULTS.max_bytes,
    archive_policy: 'archive_policy' in explicit ? explicit.archive_policy : MAILBOX_DEFAULTS.archive_policy,
    archive_retention_days: 'archive_retention_days' in explicit ? explicit.archive_retention_days : MAILBOX_DEFAULTS.archive_retention_days,
    archive_retention_count: 'archive_retention_count' in explicit ? explicit.archive_retention_count : MAILBOX_DEFAULTS.archive_retention_count,
  };
}
 ...
 * @returns {null | { ok: true, config: ProjectConfig } | ConfigErrorResult}
 */
