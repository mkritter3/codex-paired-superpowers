...
function validateUserRouting(rawRouting, recommendations, cliClients) {
  ...
  for (const [role, entry] of Object.entries(rawRouting)) {
    // Plan 3 (reviewer naming migration): the recommendation set stays keyed
    // expert-*; a reviewer-* override key canonicalizes to its expert-* twin
    // for the known-role check so reviewer override keys are accepted.
    const recKey =
      !recommendations.has(role) && /^reviewer-/.test(role)
        ? role.replace(/^reviewer-/, 'expert-')
        : role;
    if (!recommendations.has(recKey)) {
      throw new RoleRoutingError(
