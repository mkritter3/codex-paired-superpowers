// Plan 3 (reviewer naming migration) — reviewer-output-parser: strict
// JSON-block parser for reviewer turn output. Canonical home for the body that
// previously lived in expert-output-parser.js (now a shim).
//
// Reviewer turns return free-form Markdown that MUST contain one fenced JSON
// block inside a `## Machine Result` section. The Markdown around that block
// is lenient; the JSON is strict.
//
// API:
//   parseReviewerOutput(rawText, { expectedExpertId, expectedPhase })
//     -> { ok: true,  result: <parsed>, warnings?: string[] }
//     -> { ok: false, reason: <code>, ...details }
//   parseExpertOutput  — alias of parseReviewerOutput (one-window compat).
//
//   buildRepairPrompt({ rawOutput, reason, expectedExpertId, expectedPhase })
//     -> string  (one-shot repair instruction for the reviewer)

// Canonical v0.8.1 field name for the peer-DM enqueue array. Renamed from
// `peer_messages_sent` (v0.8.0) because the prior name implied the reviewer
// had already sent the messages, when in reality the orchestrator's runtime
// enqueues them on parse success. Legacy alias accepted for one release;
// removed in v0.9.0.
const PEER_FIELD = 'peer_messages_requested';
const LEGACY_PEER_FIELD = 'peer_messages_sent';
...
export function parseReviewerOutput(rawText, opts = {}) {
  ...
  // v0.8.1 — normalize legacy `peer_messages_sent` to canonical
  // `peer_messages_requested` before schema validation. The legacy alias is
  // accepted for one release so v0.8.0 transcripts and fixtures keep parsing.
  // Preferred-wins if both keys are present (legacy ignored with warning).
  const warnings = [];
  if (!(PEER_FIELD in parsed) && LEGACY_PEER_FIELD in parsed) {
    parsed[PEER_FIELD] = parsed[LEGACY_PEER_FIELD];
    warnings.push('legacy-peer_messages_sent-normalized');
  } else if (PEER_FIELD in parsed && LEGACY_PEER_FIELD in parsed) {
    warnings.push('both-peer-fields-present-preferred-wins');
  }
  ...
  if (blocks.length > 1) {
    warnings.push('multiple-machine-blocks');
  }
...
// One-window compatibility alias — SAME function reference.
export { parseReviewerOutput as parseExpertOutput };
...
export function buildRepairPrompt({
  ...
  return [
    ...
    `  - peer_messages_requested (array, can be empty; items: {to, body, summary?})`,
