...
function isNullOr(predicate) {
  return (value) => value === null || predicate(value);
}

// git rev-parse HEAD: a hex object id (abbreviated or full-length, and
// tolerant of both sha1's 40 and sha256's 64 hex chars).
const isHeadSha = (value) => typeof value === 'string' && /^[0-9a-f]{7,64}$/i.test(value);
// hashGitDiff() always produces `sha256:<64 lowercase hex chars>`.
const isDiffHash = (value) => typeof value === 'string' && /^sha256:[0-9a-f]{64}$/.test(value);
...
function isValidExitedEvidence(evidence) {
  ...
  if (!Array.isArray(evidence.changed_files) || !evidence.changed_files.every((f) => typeof f === 'string')) {
    return false;
  }
