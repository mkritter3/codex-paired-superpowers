...
export function createMailboxPoller({
  ...
  async function pollNow() {
    ...
    for (const msg of candidates) {
      ...
      // Append sidecar event FIRST. On throw: remove from inFlight + re-throw.
      // Do NOT add to deliveredIds on failure; do NOT keep in inFlightIds.
      // Listeners must NOT see the message until the audit event is durably written —
      // this enforces validation-before-delivery: no delivery without a sidecar record.
      try {
      ...
      // Only after successful sidecar write: emit to listeners, add to dedupe Set.
      // This preserves replay-truth: if emit were to throw (unlikely), the sidecar record
      // already exists and the next poll will re-emit (idempotent listener scenario).
      emit('delivered', msg);
