...
import { writeOwnershipMarker, writePreservationMarker } from '../checkout-markers.js';
 ...
 */
export async function createImplementerWorktrees({ repoRoot, sliceId, implementers, baseSha, deps = {} }) {
  // This route creates the two-disjoint fan-out. The panel guard must precede
  // input validation, filesystem checks and every git command.
  (deps.assertPanelRouteSupported ?? assertPanelRouteSupported)({ repoRoot, split: 'two-disjoint' });
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    result.set(impl.memberId, { worktreePath, branchName, baseSha, runId: sliceId });
  }
  return result;
}
 ...
 *
 * @param {Map<string, {worktreePath: string, branchName: string, runId?: string}>} worktreeMap
...
export async function cleanupImplementerWorktrees(worktreeMap, { keepForensics = false } = {}) {
  if (keepForensics) {
    for (const [, { worktreePath, branchName, runId }] of worktreeMap) {
      writePreservationMarker(worktreePath, {
        reason: 'keepForensics',
        run_id: runId || branchName,
      });
    }
    return;
  }
