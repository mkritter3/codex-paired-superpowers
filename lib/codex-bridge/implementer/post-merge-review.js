...
async function _doPostMergeReview({
  ...
  if (existingRun && Array.isArray(existingRun.events)) {
    for (const event of existingRun.events) {
      if (
        ...
        // Return the previously-recorded outcome verbatim
        const p = event.payload;
        const outcome = p.panel_status === 'panel-SHIP' ? 'ship' : 'revise';
        return {
          halted: false,
          outcome,
          findings: outcome === 'ship'
            ? [...(p.claude_nonblocking_findings || []), ...(p.codex_nonblocking_findings || [])]
            : [...(p.claude_blocking_findings || []), ...(p.codex_blocking_findings || [])],
  ...
  // Also use the aggregate's parse_failure_count if available
  const aggregateParseFailures = panelResult.aggregate?.parse_failure_count ?? 0;
  const effectiveParseFailures = Math.max(parseFailureCount, aggregateParseFailures);
