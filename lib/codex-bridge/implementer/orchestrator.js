 ...
 * }} opts
 * @returns {Promise<{success: object[], failed: object[], cancelled: object[]}>}
 */
export async function dispatchImplementers({
  ...
  // ── build the members map for startImplementerRun ────────────────────────
  const members = {};
  ...
  // ── start the run on the sidecar ─────────────────────────────────────────
  const { implementer_run_id: activeRunId } = await _startImplementerRun(specPath, sliceId, {
    base_sha: baseSha,
    members,
  });
