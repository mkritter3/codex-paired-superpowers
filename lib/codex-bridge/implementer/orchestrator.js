  ...
  async function runOne(impl) {
    ...
    try {
      ...
      if (outcome === 'failed' || outcome === 'halted') {
        // Abort siblings if this is a required member.
        // This covers the returned-failure path (not thrown); the thrown-error
        // path is handled separately in the catch block below.
      ...
      // Abort siblings if this is a required member (same rule applies).
      if (impl.required !== false) {
        abortController.abort();
      }
      return { kind: 'failed', memberId: impl.memberId, result };
