 ...
 * @returns {string[] | null}
 */
export function extractImplementersBlockLines(sliceSection) {
 ...
 */
export function parseImplementerEntries(lines) {
  const entries = [];
  let current = null;
  let inFiles = false;
  ...
  for (const rawLine of lines) {
    ...
    switch (key) {
      ...
      case 'overlap_rationale':
        ...
        break;
      case 'owner':
        current.owner = normalizeYamlScalar(value);
        break;
      default:
        break;
    }
...
function validateImplementers(entries, highCost, highCostRationale) {
  ...
  for (const entry of entries) {
    ...
    /** @type {import('./member-id.js').ParsedMemberId} */
    let parsedMemberId;
    try {
      parsedMemberId = parseMemberId(entry.member_id);
    } catch (err) {
      throw Object.assign(
    ...
    const expectedAdapter = `${parsedMemberId.cliKind}-cli`;
    if (entry.adapter !== expectedAdapter) {
      throw Object.assign(
        new Error(
          `implementer-directive-malformed: member_id parses cliKind='${parsedMemberId.cliKind}' ` +
            `but adapter='${entry.adapter}'`
        ),
        { code: 'implementer-directive-malformed' }
      );
    }

    if (entry.model !== parsedMemberId.modelId) {
      throw Object.assign(
        new Error(
          `implementer-directive-malformed: member_id parses modelId='${parsedMemberId.modelId}' ` +
            `but model='${entry.model}'`
        ),
        { code: 'implementer-directive-malformed' }
      );
    }
...
/**
 * Reject overlapping claimed files unless every member sharing a file has a
 * non-empty `overlap_rationale`.
 *
 * This is the single source of truth for claimed-file overlap rejection. It is
 * called by `validateImplementers` (implementer-experts) and reused by hybrid
 * ownership validation — there must not be a second overlap checker with
 * divergent behavior (spec §5 closing requirement).
 *
 * @param {ImplementerEntry[]} entries
 * @throws {Error} with `.code = 'implementer-claimed-files-missing'` on an
 *   unrationalized overlap.
 */
export function validateClaimedFileOverlap(entries) {
