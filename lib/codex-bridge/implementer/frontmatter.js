 ...
 * Returns the lines of the block body (after the label), or null if absent.
 *
...
function parseImplementerEntries(lines) {
  ...
  for (const rawLine of lines) {
    ...
    if (/^-\s+member_id\s*:/.test(trimmed)) {
      flush();
      const value = trimmed.replace(/^-\s+member_id\s*:\s*/, '').trim();
      current = {
        member_id: value,
        adapter: '',
        model: '',
    ...
    if (inFiles) {
      if (/^-\s+/.test(trimmed)) {
        const filePath = trimmed.replace(/^-\s+/, '').trim();
    ...
    switch (key) {
      case 'adapter':
        current.adapter = value.trim();
        break;
      case 'model':
        current.model = value.trim();
        break;
      case 'required':
        current.required = value.trim().toLowerCase() !== 'false';
        break;
      case 'files':
        inFiles = true;
        // If files: has an inline value (uncommon), treat it as a file.
        if (value.trim().length > 0) {
          current.files.push(value.trim());
        }
        break;
      case 'overlap_rationale':
        current.overlap_rationale = value.trim();
        break;
      default:
        break;
    }
