...
async function _tryAuditDivergenceRecovery({
  ...
  // Find commits with Merger-Member-Id matching our mergerMemberId
  // (most recent first — pick the first match)
  let recoveryCommit = null;
  for (const commit of commits) {
    const memberIdMatch = MERGER_MEMBER_ID_TRAILER_RE.exec(commit.body);
  ...
  if (alreadyHasAudit) {
    // Idempotency: audit already exists, no recovery needed
    return null;
  }

  // Recovery case: we have the commit but missing the audit event.
  // Parse trailers from the commit body.
  const claudeMatch = CLAUDE_REVIEW_TRAILER_RE.exec(commitBody);
  const codexMatch = CODEX_REVIEW_TRAILER_RE.exec(commitBody);
  const claudeVerdict = claudeMatch ? claudeMatch[1].trim() : null;
  const codexVerdict = codexMatch ? codexMatch[1].trim() : null;

  // If trailers are present but NOT exactly 'SHIP' → audit-divergence
  if (
    ...
    return {
      ...
      mergerCommitSha,
      diagnostic: `Recovery scan found merge-resolution commit but Claude-Review="${claudeVerdict}" or Codex-Review="${codexVerdict}" is not SHIP`,
    };
  }

  // Re-append the missing merger_completed.outcome:'committed' audit event.
  // We use SHIP for both verdicts (trailers present and valid, or absent — treated as SHIP
  // since the commit was already pushed through the double-SHIP gate).
  const effectiveClaude = claudeVerdict || 'SHIP';
  const effectiveCodex = codexVerdict || 'SHIP';
