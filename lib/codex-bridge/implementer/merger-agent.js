...
async function _tryAuditDivergenceRecovery({
  ...
  let malformedCandidate = null; // first commit that passes subject+id checks but has bad review trailers
  ...
  for (const commit of commits) {
    ...
    // (do not silently accept; record for halt after loop)
    if (
      (claudeMatch && claudeVerdict !== 'SHIP') ||
      (codexMatch && codexVerdict !== 'SHIP')
    ) {
      if (!malformedCandidate) {
        malformedCandidate = { commit, claudeVerdict, codexVerdict };
      }
      continue; // do not use as recovery candidate
    }
  ...
  // If we found a malformed candidate (bad review trailers) before any valid
  // candidate, halt with audit-divergence rather than silently falling through.
  if (!recoveryCommit && malformedCandidate) {
    const { commit, claudeVerdict, codexVerdict } = malformedCandidate;
    return {
      halted: true,
      halt: 'merger-audit-divergence',
      mergerCommitSha: commit.sha,
      diagnostic: `Recovery scan found malformed recovery trailers: Claude-Review="${claudeVerdict}" or Codex-Review="${codexVerdict}" is not SHIP`,
    };
  }
