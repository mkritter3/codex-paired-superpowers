...
import { redactCanaryTokensInString } from './secret-redaction.js';
...
function composePrompt({ mergeContext, conflictedFiles, allowlist, promptByteCap }) {
  ...
  // Redact any canary substrings from the composed prompt before dispatch.
  // Use redactCanaryTokensInString (substring replacement) rather than
  // redactSecretFields (whole-string replacement) so that a single leaked
  // canary token cannot collapse the entire 100KB prompt to '<REDACTED>'.
  const prompt = redactCanaryTokensInString(rawPrompt);
