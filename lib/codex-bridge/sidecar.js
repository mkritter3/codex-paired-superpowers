...
export function assertRoundShipBacked(round, scope, { headSha, source = 'appendRoundWithAudits' } = {}) {
  ...
  const shaEntries = codeBearing ? roundEntries.filter((a) => a.reviewed_sha !== undefined) : [];

  if (shaEntries.length > 0) {
    if (typeof headSha !== 'string' || headSha.length === 0) {
      throw new Error(`${source}: headSha required when review audits carry reviewed_sha`);
    }
    const reviewedShas = new Set(shaEntries.map((a) => a.reviewed_sha));
    if (reviewedShas.size !== 1) {
      throw new Error(`${source}: all audits for the round must use the same reviewed_sha`);
    }
    const [reviewedSha] = reviewedShas;
    if (headSha !== reviewedSha) {
      throw new Error(`${source}: headSha ${headSha} does not match reviewed_sha ${reviewedSha}`);
    }
  }
    ...
    if (codeBearing) {
      ...
      if (shaEntries.length > 0) {
        if (qualifying.some((a) => a.reviewed_sha === undefined)) {
          throw new Error(`${source}: ${side} qualifying verification audit must include reviewed_sha`);
        }
        if (!qualifying.some((a) => a.reviewed_sha === headSha)) {
          throw new Error(`${source}: ${side} has no qualifying verification audit at headSha ${headSha}`);
        }
      }
