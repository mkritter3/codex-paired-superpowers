...
function assertRoundShipBacked(round, scope) {
  ...
  for (const side of ['claude', 'codex']) {
    ...
    if (codeBearing) {
      const ok = entries.some((a) => Array.isArray(a.commands) && a.commands.some((c) => c.kind === 'verification' && c.exit_code === 0));
      if (!ok) {
        throw new Error(`appendRoundWithAudits: ${side} SHIP in code-bearing phase ${round.phase} requires a successful kind:"verification" command (exit_code 0)`);
      }
