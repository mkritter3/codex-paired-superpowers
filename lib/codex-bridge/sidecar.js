...
// @ts-expect-error proper-lockfile is a vendored CommonJS runtime dependency without declarations.
import * as lockfile from 'proper-lockfile';
import { containsCanary } from './implementer/secret-redaction.js';
import { MODEL_ROLES, VALID_CLIS, SAFE_TOKEN, roleForThread } from './models.js';

/** @typedef {Record<string, any>} JsonObject */
/** @typedef {import('./types.js').Round} Round */
/** @typedef {import('./types.js').ThreadConfig} ThreadConfig */
/**
 * @typedef {object} ImplementMeta
 * @property {string} preferred_implementer
 * @property {string} fallback_implementer
 * @property {string | null} parallel_group
 * @property {string | null} parallel_suppressed_reason
 * @property {string} worktree
 * @property {string} [domain]
 */
/** @typedef {{symlinks: string[], completed_at: string}} ImplementBootstrap */
/** @typedef {{id: string, role: string, source: string, phase: string, selectionReason: string}} ReviewerSelection */
/** @typedef {{phase: string, selected_count: number, rationale: string}} FanOutRationale */
/** @typedef {{digest: string, dag: object}} DependencyGraph */
/** @typedef {{sliceId: string, dispatched_at: string}} DispatchLocator */
/** @typedef {{disposition: string, rationale?: string, evidence?: string[]}} BlockerUpdate */
/**
 * @typedef {object} ImplementTerminal
 * @property {string} outcome
 * @property {string} completed_at
 * @property {string} [head_sha]
 * @property {number} [commit_count]
 * @property {string[]} [concerns]
 * @property {string} [model_role]
 * @property {string} [model]
 * @property {string} [effort]
 */
/** @typedef {{block: string}} GoalsBlock */
/** @typedef {{phase?: string, round?: number, side?: string}} AuditFilter side is compared by equality; an unknown side matches nothing */
/**
 * @typedef {object} AuditSelection
 * @property {string} mode
 * @property {number} ran
 * @property {boolean} [fullyCovered]
 * @property {unknown[]} [uncovered]
 * @property {number} [exit]
 */
/**
 * @typedef {object} AuditCommand
 * @property {string} cmd
 * @property {string} summary
 * @property {'inspection' | 'verification' | 'other'} kind
 * @property {number} [exit_code]
 * @property {AuditSelection} [selection]
 ...
 * @property {AuditCommand[]} commands
 * @property {string} [verdict_basis]
 * @property {string} [reviewed_sha]
 */
/**
 * @typedef {object} ImplementerMember
 * @property {string} adapter
 * @property {string} model
 * @property {boolean} required
 * @property {string} worktree_id
 * @property {string} branch
 * @property {string[]} claimed_files
 * @property {'claude-ui' | 'codex-backend'} [owner]
 * @property {string} [runtime_kind]
 * @property {string} [overlap_rationale]
 */
/** @typedef {{base_sha: string, members: Record<string, ImplementerMember>}} ImplementerRun */
/**
 * @typedef {object} ImplementerRunResult
 * @property {string} status
 * @property {string} [completed_at]
 * @property {object | null} [merge]
 * @property {object | null} [post_merge_review]
 */
...
export class SidecarLockError extends Error {
  /** @param {string} message @param {{code?: string, cause?: unknown}} [options] */
...
/** @param {string} specPath @returns {string} */
...
/** @param {string} specPath @param {{feature: string, codexSession: string, model: string, reasoningEffort: string}} options */
export function initSidecar(specPath, { feature, codexSession, model, reasoningEffort }) {
  ...
  const data = {
    ...
    thread_config: {
      'paired-reviewer': {
        role: 'planning', cli: 'codex', model, effort: reasoningEffort, opened_at: openedAt,
      },
    },
    rounds: [],
    panel_roster: {},
...
/** @param {string} specPath @returns {JsonObject} */
 ...
 *
 * @param {JsonObject} sidecarData — parsed sidecar JSON (mutated in place)
...
function migrateIfNeeded(sidecarData, specPath) {
  ...
  // Plan 3 (reviewer rename) — silent, idempotent migration of the v0.8.0
  // `expert_teammates` block to the canonical `reviewer_teammates` block.
  // Guard: only when the old block exists AND the new one does not (so a
  // sidecar that already carries both — distinct contents — is left intact
  // and reviewer_teammates wins via getTeammatesBlock precedence). The legacy
  // expert_teammates is PRESERVED for one migration window; readers go through
  // getTeammatesBlock, which prefers reviewer_teammates.
  if (sidecarData.expert_teammates && !sidecarData.reviewer_teammates) {
    sidecarData.reviewer_teammates = JSON.parse(
      JSON.stringify(sidecarData.expert_teammates)
    );
    if (!Array.isArray(sidecarData.migrations)) {
      sidecarData.migrations = [];
    }
    sidecarData.migrations.push({
      from_schema: 'expert_teammates',
      to_schema: 'reviewer_teammates',
      action: 'expert_teammates → reviewer_teammates',
      migrated_at: new Date().toISOString(),
    });
    migrated = true;
  }
 ...
 * role_sessions entry.
 *
 * @param {JsonObject} sidecarData — already-loaded sidecar JSON
...
/**
 * Return the immutable model snapshot for a named thread, with a role-aware
 * legacy marker when no snapshot was recorded (v0.16.0 spec §5).
 */
/** @param {JsonObject} sidecarData @param {string} [key] @returns {ThreadConfig} */
export function getThreadConfig(sidecarData, key = 'paired-reviewer') {
  const entry = sidecarData?.thread_config?.[key];
  if (entry) {
    if (entry.cli === undefined) {
      return { ...entry, cli: 'codex' };
    }
    return entry;
  }
  return { role: roleForThread(key), cli: 'codex', model: null, effort: null, legacy: true };
}

/** @param {string} specPath @param {JsonObject} data */
...
/** @param {JsonObject} deps @returns {JsonObject} */
...
/** @param {string} target @param {JsonObject} options */
...
/** @template T @param {string} specPath @param {() => Promise<T> | T} fn @param {JsonObject} [deps] @returns {Promise<T>} */
...
// v0.15.0 — the per-phase review-round budget the skills enforce in prose
// ("7-round Claude↔Codex loop"). Sidecar replay across 24 features showed the
// prose cap is leaky in both directions: plans ran to 11/13/15 rounds by
// appending new logical work under the same phase key, and counter
// discontinuities (r4 → r11) slipped through unnoticed. The sink is where the
// budget actually binds.
export const ROUND_BUDGET = 7;

/**
 * v0.15.0 — validate a round entry before it is appended.
 *
 * Shape is checked unconditionally: a bad caller once pushed bare integers
 * into rounds[], which broke every later consumer that indexed round.codex.
 *
 * `opts.sequential` — round must be exactly max(existing for phase) + 1.
 * `opts.budget` — refuse rounds beyond ROUND_BUDGET; new logical work gets a
 *   new phase key (plan-2, ...), an over-budget continuation needs explicit
 *   user approval (cli --allow-over-budget).
 *
 * @param {JsonObject} sc — loaded sidecar (rounds may be absent on legacy files)
 * @param {JsonObject} round
 * @param {{sequential?: boolean, budget?: boolean}} [opts]
 */
export function validateRoundForAppend(sc, round, opts = {}) {
  if (!round || typeof round !== 'object' || Array.isArray(round)) {
    throw new Error('appendRound: round must be a plain object, got ' + JSON.stringify(round));
  }
  if (typeof round.phase !== 'string' || round.phase.length === 0) {
    throw new Error('appendRound: round.phase must be a non-empty string');
  }
  if (!Number.isInteger(round.round) || round.round < 1) {
    throw new Error(`appendRound: round.round must be a positive integer, got ${JSON.stringify(round.round)}`);
  }
  const existing = (Array.isArray(sc.rounds) ? sc.rounds : [])
    .filter((/** @type {JsonObject} */ r) => r && typeof r === 'object' && r.phase === round.phase);
  const maxExisting = existing.reduce(
    (/** @type {number} */ m, /** @type {JsonObject} */ r) => (Number.isInteger(r.round) ? Math.max(m, r.round) : m), 0,
  );
  if (opts.sequential) {
    const expected = maxExisting + 1;
    if (round.round !== expected) {
      throw new Error(
        `appendRound: phase "${round.phase}" expects round ${expected} next (last recorded: ${maxExisting}), got ${round.round}. ` +
        'If this is genuinely new logical work, use a fresh phase key instead of reusing this one; ' +
        'to force a non-sequential append after user approval, pass --force-round.',
      );
    }
  }
  if (opts.budget && round.round > ROUND_BUDGET) {
    throw new Error(
      `appendRound: round ${round.round} exceeds the ${ROUND_BUDGET}-round budget for phase "${round.phase}". ` +
      'The loop protocol requires surfacing the disagreement to the user at the cap. ' +
      'Start a new phase key (e.g. "plan-2") for new logical work, or pass --allow-over-budget ' +
      'after the user explicitly approves continuing this loop.',
    );
  }
}

// v0.15.0 — design phases under the SHIP-audit gate. `plan-2`, `plan-3`, ...
// are first-class now that the budget pushes new logical plans onto fresh keys.
const DESIGN_PHASE_RE = /^(?:spec|plan(?:-\d+)?|plan-slice:.+)$/;

/**
 * Map a persisted round phase to the configured review-panel phase that owns it.
 * Keep planning aligned with DESIGN_PHASE_RE so budget-driven plan-N keys cannot
 * escape a roster that was fixed at the start of planning.
 *
 * @param {unknown} roundPhase
 * @returns {'planning' | 'review' | null}
 */
export function panelPhaseForRound(roundPhase) {
  if (typeof roundPhase !== 'string') return null;
  if (DESIGN_PHASE_RE.test(roundPhase)) return 'planning';
  if (/^review-slice:.+/.test(roundPhase) || roundPhase === 'docs-update') return 'review';
  return null;
}

/** @param {string} code @param {string} reason @returns {Error & {code: string, reason: string}} */
function panelError(code, reason) {
  const error = /** @type {Error & {code: string, reason: string}} */ (new Error(`${code}: ${reason}`));
  error.code = code;
  error.reason = reason;
  return error;
}
...
function normalizedArtifactVersion(value) {
...
/** @param {unknown} value @returns {'SHIP' | 'REVISE' | null} */
function verdictStatus(value) {
  if (typeof value !== 'string') return null;
  if (/^\s*SHIP(?![A-Z0-9_])/.test(value)) return 'SHIP';
  if (/^\s*REVISE(?![A-Z0-9_])/.test(value)) return 'REVISE';
  return null;
}

/** @param {unknown} roster @param {string} source @returns {JsonObject[]} */
function validatePanelRoster(roster, source) {
  if (!Array.isArray(roster) || roster.length === 0) {
    throw panelError('panel-roster-changed', `${source} must be a non-empty array`);
  }
  const seen = new Set();
  return roster.map((entry, index) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
      throw panelError('panel-roster-changed', `${source}[${index}] must be a plain object`);
    }
    for (const field of ['member_id', 'cli', 'model', 'effort']) {
      if (typeof entry[field] !== 'string' || entry[field].length === 0) {
        throw panelError('panel-roster-changed', `${source}[${index}].${field} must be a non-empty string`);
      }
    }
    if (!/^(codex|agy):[A-Za-z0-9._-]+$/.test(entry.member_id)) {
      throw panelError('panel-roster-changed', `${source}[${index}].member_id is invalid`);
    }
    if (entry.member_id !== `${entry.cli}:${entry.model}`) {
      throw panelError('panel-roster-changed', `${source}[${index}].member_id must equal cli:model`);
    }
    if (seen.has(entry.member_id)) {
      throw panelError('panel-roster-changed', `${source} contains duplicate member ${entry.member_id}`);
    }
    seen.add(entry.member_id);
    return { member_id: entry.member_id, cli: entry.cli, model: entry.model, effort: entry.effort };
  });
}

/**
 * Persist a phase roster once. Repeating the same roster is idempotent; any
 * later change fails closed so one phase never mixes reviewer identities.
 *
 * @param {string} specPath
 * @param {'planning' | 'review'} phase
 * @param {unknown} roster
 */
export function setPanelRoster(specPath, phase, roster) {
  if (phase !== 'planning' && phase !== 'review') {
    throw panelError('panel-roster-changed', `phase must be planning or review; got ${JSON.stringify(phase)}`);
  }
  const normalized = validatePanelRoster(roster, `panel_roster.${phase}`);
  const sc = loadSidecar(specPath);
  const existing = sc.panel_roster?.[phase];
  if (existing !== undefined && JSON.stringify(existing) !== JSON.stringify(normalized)) {
    throw panelError('panel-roster-changed', `${phase} roster differs from the persisted roster`);
  }
  if (existing !== undefined) return;
  if (!sc.panel_roster || typeof sc.panel_roster !== 'object' || Array.isArray(sc.panel_roster)) {
    sc.panel_roster = {};
  }
  sc.panel_roster[phase] = normalized;
  saveSidecar(specPath, sc);
}

/** @param {JsonObject} sc @param {JsonObject} round @returns {JsonObject[] | null} */
function panelRosterForRound(sc, round) {
  const phase = panelPhaseForRound(round.phase);
  if (phase === null) return null;
  const roster = sc.panel_roster?.[phase];
  return Array.isArray(roster) ? roster : null;
}

/**
 * Enforce the configured panel contract at the common append boundary. The
 * aggregate Codex verdict is derived here so downstream readers never trust a
 * caller-supplied vote that disagrees with the persisted member records.
 *
 * @param {JsonObject} sc
 * @param {JsonObject} round
 */
export function validatePanelRound(sc, round) {
  const roster = panelRosterForRound(sc, round);
  if (roster === null) {
    // Panel fields are only meaningful against a persisted roster. Accepting them here would let a
    // caller-supplied `panel` choose which sides assertRoundShipBacked demands audits for.
    if (round.panel !== undefined || round.claude_version !== undefined) {
      throw panelError('panel-round-invalid', `panel fields supplied for phase ${round.phase}, which has no panel roster`);
    }
    return;
  }
  if (!Array.isArray(round.panel)) {
    throw panelError('panel-round-invalid', `panel is required for rostered phase ${round.phase}`);
  }

  const rosterById = new Map(roster.map((entry) => [entry.member_id, entry]));
  const panelById = new Map();
  for (const [index, entry] of round.panel.entries()) {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
      throw panelError('panel-round-invalid', `panel[${index}] must be a plain object`);
    }
    const unexpected = Object.keys(entry).filter(
      (field) => !['member_id', 'cli', 'model', 'version', 'status', 'session'].includes(field),
    );
    if (unexpected.length > 0) {
      throw panelError('panel-round-invalid', `panel[${index}] contains unexpected field ${unexpected[0]}`);
    }
    const memberId = entry.member_id;
    if (typeof memberId !== 'string' || memberId.length === 0) {
      throw panelError('panel-round-invalid', `panel[${index}].member_id must be a non-empty string`);
    }
    if (panelById.has(memberId)) {
      throw panelError('panel-round-invalid', `duplicate member ${memberId}`);
    }
    if (!rosterById.has(memberId)) {
      throw panelError('panel-round-invalid', `extra member ${memberId}`);
    }
    const rosterEntry = /** @type {JsonObject} */ (rosterById.get(memberId));
    if (entry.cli !== rosterEntry.cli || entry.model !== rosterEntry.model) {
      throw panelError('panel-round-invalid', `${memberId} cli/model does not match the persisted roster`);
    }
    if (normalizedArtifactVersion(entry.version) === null) {
      throw panelError('panel-round-invalid', `${memberId} version must be a non-empty string`);
    }
    if (entry.status !== 'SHIP' && entry.status !== 'REVISE') {
      throw panelError('panel-round-invalid', `${memberId} status must be SHIP or REVISE`);
    }
    if (typeof entry.session !== 'string' || entry.session.length === 0) {
      throw panelError('panel-round-invalid', `${memberId} session must be a non-empty string`);
    }
    panelById.set(memberId, entry);
  }

  for (const memberId of rosterById.keys()) {
    if (!panelById.has(memberId)) {
      throw panelError('panel-round-invalid', `missing member ${memberId}`);
    }
  }

  // Canonicalize Claude's verdict like the derived codex one, so the persisted value, the audited
  // SHIP sides, and downstream prefix readers (the release gate's startsWith('SHIP')) all agree.
  const claudeStatus = verdictStatus(round.claude);
  if (claudeStatus === null || !/^\s*(?:SHIP|REVISE)\s*$/.test(String(round.claude))) {
    throw panelError('panel-round-invalid', `claude verdict must be SHIP or REVISE; got ${JSON.stringify(round.claude)}`);
  }
  round.claude = claudeStatus;

  const claudeVersion = normalizedArtifactVersion(round.claude_version);
  if (claudeVersion === null) {
    throw panelError('panel-round-invalid', 'claude_version must be a non-empty string');
  }
  for (const [memberId, entry] of panelById) {
    if (normalizedArtifactVersion(entry.version) !== claudeVersion) {
      throw panelError('panel-round-invalid', `${memberId} version does not match claude_version`);
    }
  }

  const derived = [...panelById.values()].every((entry) => entry.status === 'SHIP') ? 'SHIP' : 'REVISE';
  if (round.codex !== undefined) {
    const supplied = verdictStatus(round.codex);
    if (supplied === null || supplied !== derived) {
      throw panelError('panel-round-invalid', `caller codex verdict ${JSON.stringify(round.codex)} disagrees with derived ${derived}`);
    }
  }
  round.codex = derived;
}
...
/**
 * v0.15.0 — the SHIP-audit gate, relocated from the PreToolUse hook's
 * shell-string regex (which was fail-open against `--round "$VAR"`,
 * $(...), and heredoc forms) into the sink where the round arrives parsed.
 * Design phases need an audit entry per SHIP side; code-bearing phases need
 * a successful executed verification. Phases outside both classes pass.
 */
/** @param {JsonObject} sc @param {JsonObject} round @param {{headSha?: string}} [opts] */
export function assertShipAuditsForAppend(sc, round, opts = {}) {
  return assertRoundShipBacked(round, Array.isArray(sc.audits) ? sc.audits : [], {
    headSha: opts.headSha,
    source: 'appendRound',
  });
}

/** @param {string} specPath @param {JsonObject} round @param {{enforceShipAudits?: boolean, headSha?: string, sequential?: boolean, budget?: boolean}} [opts] */
export function appendRound(specPath, round, opts = {}) {
  const sc = loadSidecar(specPath);
  validateRoundForAppend(sc, round, opts);
  validatePanelRound(sc, round);
  if (opts.enforceShipAudits || panelRosterForRound(sc, round) !== null) {
    assertShipAuditsForAppend(sc, round, { headSha: opts.headSha });
  }
  sc.rounds.push(round);
  saveSidecar(specPath, sc);
}

/** @param {string} specPath @param {string} sliceId @param {JsonObject} sliceState */
...
/** @param {string} specPath @param {string} sliceId @param {string} phaseName @param {JsonObject} phaseState */
...
/**
 * Append a fix-pass checkpoint under the review-slice phase (v0.16.0 spec §6 step 3).
 */
/** @param {string} specPath @param {string} sliceId @param {JsonObject} fixPass */
export function appendFixPass(specPath, sliceId, fixPass) {
  const sc = loadSidecar(specPath);
  if (!sc.slice_reviews[sliceId]) sc.slice_reviews[sliceId] = { phases: {} };
  if (!sc.slice_reviews[sliceId].phases) sc.slice_reviews[sliceId].phases = {};
  if (!sc.slice_reviews[sliceId].phases['review-slice']) {
    sc.slice_reviews[sliceId].phases['review-slice'] = {};
  }
  const phase = sc.slice_reviews[sliceId].phases['review-slice'];
  if (!Array.isArray(phase.fix_passes)) phase.fix_passes = [];
  phase.fix_passes.push({ ...fixPass });
  saveSidecar(specPath, sc);
  return phase.fix_passes.length - 1;
}

/**
 * Merge terminal evidence into an existing fix-pass checkpoint (v0.16.0 spec §6 step 3).
 */
/** @param {string} specPath @param {string} sliceId @param {number} index @param {JsonObject} updates */
export function updateFixPass(specPath, sliceId, index, updates) {
  const sc = loadSidecar(specPath);
  const passes = sc.slice_reviews?.[sliceId]?.phases?.['review-slice']?.fix_passes;
  if (!Array.isArray(passes) || !passes[index]) {
    throw new Error(`updateFixPass: no fix pass at index ${index} for ${sliceId}`);
  }
  Object.assign(passes[index], updates);
  saveSidecar(specPath, sc);
}
...
/**
 * v0.14.0 hybrid: write the hybrid status block under the slice phase
 * (spec §9). This is an additive convenience mirror — it lives at
 * `phases.hybrid` and never touches `phases.implementer_experts`, which
 * remains the durable event-stream audit trail. Lazy-inits intermediate
 * objects (same pattern as setPhase / setLiveVerification).
 *
 * @param {string} specPath
 * @param {string} sliceId — e.g. "slice-4"
 * @param {JsonObject} block — the §9 hybrid status block (owners + contract sync state)
 */
export function setHybridStatus(specPath, sliceId, block) {
  const sc = loadSidecar(specPath);
  if (!sc.slice_reviews[sliceId]) sc.slice_reviews[sliceId] = { phases: {} };
  if (!sc.slice_reviews[sliceId].phases) sc.slice_reviews[sliceId].phases = {};
  sc.slice_reviews[sliceId].phases.hybrid = block;
  saveSidecar(specPath, sc);
}

/**
 * Read the hybrid status block for a slice. Returns null when absent (no slice,
 * no phases, or no hybrid block).
 *
 * @param {string} specPath
 * @param {string} sliceId — e.g. "slice-4"
 * @returns {object | null}
 */
export function getHybridStatus(specPath, sliceId) {
  const sc = loadSidecar(specPath);
  return sc.slice_reviews?.[sliceId]?.phases?.hybrid ?? null;
}
 ...
 */
/** @param {string} specPath @param {string} sliceId @param {JsonObject} round */
 ...
 */
/** @param {string} specPath @param {string} sliceId @param {string} scenarioId @param {string} attemptKey @param {JsonObject} result */
...
/** @param {JsonObject} sc @param {string} sliceId @returns {JsonObject} */
 ...
 * @param {string} specPath
 * @param {string} sliceId — e.g. "slice-3"
 * @param {ImplementBootstrap} bootstrap
 */
 ...
 */
/** @param {string} specPath @param {string} sliceId @param {JsonObject} dispatch */
export function appendImplementDispatch(specPath, sliceId, dispatch) {
  ...
  if (dispatch.model_role !== undefined && dispatch.model_role !== null && !MODEL_ROLES.includes(dispatch.model_role)) {
    throw new Error(
      `appendImplementDispatch: invalid model_role ${JSON.stringify(dispatch.model_role)} — must be one of ${MODEL_ROLES.join(', ')}`,
    );
  }
  for (const field of ['model', 'effort']) {
    if (dispatch[field] !== undefined && dispatch[field] !== null &&
        (typeof dispatch[field] !== 'string' || !SAFE_TOKEN.test(dispatch[field]))) {
      throw new Error(`appendImplementDispatch: ${field} must be a non-empty safe token when present`);
    }
  }
  if (dispatch.rung !== undefined && dispatch.rung !== null &&
      (!Number.isInteger(dispatch.rung) || dispatch.rung < 1 || dispatch.rung > 3)) {
    throw new Error('appendImplementDispatch: rung must be an integer from 1 through 3 when present');
  }
  ...
  // Plan 3 (reviewer rename) — canonical reviewers_selected / reviewer_turn_ids
  // / reviewer_blockers accepted alongside the legacy expert_* names (one
  // migration window; both round-trip unchanged).
  // Null/undefined treated as "field absent" for back-compat with pre-0.8.0
  // records. Empty arrays allowed.
  for (const field of ['experts_selected', 'expert_turn_ids', 'reviewers_selected', 'reviewer_turn_ids']) {
  ...
  // Plan 3 (reviewer rename) — canonical reviewer_blockers (parallel shape to
  // expert_blockers but keyed by reviewer_id). Accepted on write; round-trips
  // unchanged.
  if ('reviewer_blockers' in dispatch &&
      dispatch.reviewer_blockers !== null &&
      dispatch.reviewer_blockers !== undefined) {
    if (!Array.isArray(dispatch.reviewer_blockers)) {
      throw new Error(
        `appendImplementDispatch: reviewer_blockers must be an array when present (got ${typeof dispatch.reviewer_blockers})`
      );
    }
    for (const b of dispatch.reviewer_blockers) {
      if (b === null || typeof b !== 'object' || Array.isArray(b)) {
        throw new Error(
          `appendImplementDispatch: reviewer_blockers elements must be objects (got ${JSON.stringify(b)})`
        );
      }
      for (const required of ['reviewer_id', 'finding_id', 'summary', 'location', 'disposition']) {
        if (typeof b[required] !== 'string' || b[required].length === 0) {
          throw new Error(
            `appendImplementDispatch: reviewer_blockers element missing required string field "${required}"`
          );
        }
      }
      if (b.disposition !== 'open') {
        throw new Error(
          `appendImplementDispatch: reviewer_blockers element disposition must be "open" on initial write (got "${b.disposition}"). ` +
            `Use updateDispatchExpertBlocker to transition.`
        );
      }
    }
...
// Plan 3 (reviewer rename) — reviewer_teammates is the canonical block. New
// writes land here; the legacy expert_teammates block is migrated on load via
// migrateIfNeeded and read through getTeammatesBlock.
/** @param {JsonObject} sc @returns {JsonObject} */
function ensureReviewerTeammatesBlock(sc) {
  if (!sc.reviewer_teammates) {
    sc.reviewer_teammates = {
      selected: [],
      turns: [],
      fan_out_rationales: [],
    };
  }
  if (!Array.isArray(sc.reviewer_teammates.selected)) sc.reviewer_teammates.selected = [];
  if (!Array.isArray(sc.reviewer_teammates.turns)) sc.reviewer_teammates.turns = [];
  if (!Array.isArray(sc.reviewer_teammates.fan_out_rationales)) {
    sc.reviewer_teammates.fan_out_rationales = [];
  }
  return sc.reviewer_teammates;
}

/**
 * Plan 3 (reviewer rename) — pure read accessor for the teammates block.
 * Prefers the canonical reviewer_teammates; falls back to the legacy
 * expert_teammates for sidecars not yet migrated in memory. NEVER mutates the
 * sidecar (no migration record append) — only loadSidecar/migrateIfNeeded
 * persist the migration.
 *
 * @param {object} sidecar — parsed sidecar JSON
 * @returns {object | undefined}
 */
/** @param {JsonObject} sidecar @returns {JsonObject | undefined} */
export function getTeammatesBlock(sidecar) {
  if (!sidecar || typeof sidecar !== 'object') return undefined;
  return sidecar.reviewer_teammates ?? sidecar.expert_teammates;
}
...
/**
 * Append a reviewer selection record to reviewer_teammates.selected[].
 * Initial status is "active". Source must be in {builtin, repo-override}.
 *
 * @param {string} specPath
 * @param {ReviewerSelection} sel
 */
export function appendReviewerSelection(specPath, sel) {
  if (!sel || typeof sel !== 'object') {
    throw new Error('appendReviewerSelection: selection must be an object');
  }
  if (!nonEmptyString(sel.id)) {
    throw new Error('appendReviewerSelection: id must be a non-empty string');
  }
  if (!nonEmptyString(sel.role)) {
    throw new Error('appendReviewerSelection: role must be a non-empty string');
  }
  if (!VALID_EXPERT_SOURCES.has(sel.source)) {
    throw new Error(
      `appendReviewerSelection: invalid source "${sel.source}" — must be "builtin" or "repo-override"`
    );
  }
  if (!nonEmptyString(sel.phase)) {
    throw new Error('appendReviewerSelection: phase must be a non-empty string');
  }
  if (!nonEmptyString(sel.selectionReason)) {
    throw new Error('appendReviewerSelection: selectionReason must be a non-empty string');
  }
  const sc = loadSidecar(specPath);
  const block = ensureReviewerTeammatesBlock(sc);
  block.selected.push({
    id: sel.id,
    role: sel.role,
    source: sel.source,
...
/**
 * Plan 3 (reviewer rename) — back-compat shim. appendExpertSelection delegates
 * to the canonical reviewer API so old callers converge on reviewer_teammates.
 */
/** @param {string} specPath @param {ReviewerSelection} sel */
export function appendExpertSelection(specPath, sel) {
  return appendReviewerSelection(specPath, sel);
}

/**
 * Append a turn record to reviewer_teammates.turns[].
 *
 * Internal validation error messages retain the historical "appendExpertTurn:"
 * prefix so existing field-level assertions keep matching; the canonical name
 * is appendReviewerTurn.
 *
 * @param {string} specPath
 * @param {object} turn
 */
/** @param {string} specPath @param {JsonObject} turn */
export function appendReviewerTurn(specPath, turn) {
  ...
  const modelRoleWarning = validateStringField('model_role_warning');
  let modelRole = null;
  if ('model_role' in turn && turn.model_role !== undefined && turn.model_role !== null) {
    if (!MODEL_ROLES.includes(turn.model_role)) {
      throw new Error(
        `appendExpertTurn: model_role must be one of ${MODEL_ROLES.join(', ')} when present (got ${JSON.stringify(turn.model_role)})`,
      );
    }
    modelRole = turn.model_role;
  }
  ...
  if (modelRole !== null) turnRecord.model_role = modelRole;
  if (modelRoleWarning !== null) turnRecord.model_role_warning = modelRoleWarning;
...
/**
 * Plan 3 (reviewer rename) — back-compat shim. appendExpertTurn delegates to
 * the canonical reviewer API so old callers converge on reviewer_teammates.
 */
/** @param {string} specPath @param {JsonObject} turn */
export function appendExpertTurn(specPath, turn) {
  return appendReviewerTurn(specPath, turn);
}
 ...
 */
/** @param {string} specPath @param {JsonObject} turn @param {JsonObject} [deps] */
export async function appendReviewerTurnLocked(specPath, turn, deps = {}) {
...
/**
 * Plan 3 (reviewer rename) — back-compat shim for the locked write path.
 */
/** @param {string} specPath @param {JsonObject} turn @param {JsonObject} [deps] */
export async function appendExpertTurnLocked(specPath, turn, deps = {}) {
  return appendReviewerTurnLocked(specPath, turn, deps);
}
...
export function updateExpertStatus(specPath, expertId, status) {
  ...
  const sc = loadSidecar(specPath);
  const block = ensureReviewerTeammatesBlock(sc);
  const entry = block.selected.find((/** @type {JsonObject} */ e) => e.id === expertId);
  if (!entry) {
 ...
 */
/**
 * Implementation signature (hidden by the overload): the default is typed honestly as optional;
 * the runtime check below still rejects a missing phase exactly as before.
 * @param {string} specPath
 * @param {{phase?: string, sliceId?: string}} [filter]
 * @returns {object[]}
 */
export function readReviewerTurns(specPath, filter = {}) {
  return readReviewerTurnsImpl(specPath, filter);
}

/**
 * Shared implementation for both readers: honestly typed optional filter; `phase` is validated
 * at runtime here, so neither public shim needs to assert it.
 * @param {string} specPath
 * @param {{phase?: string, sliceId?: string}} filter
 * @returns {object[]}
 */
function readReviewerTurnsImpl(specPath, filter) {
  if (!filter || typeof filter !== 'object') {
    throw new Error('readReviewerTurns: filter must be an object');
  }
  if (!nonEmptyString(filter.phase)) {
    throw new Error('readReviewerTurns: filter.phase must be a non-empty string');
  }
  const sc = loadSidecar(specPath);
  const turns = getTeammatesBlock(sc)?.turns;
...
/**
 * Plan 3 (reviewer rename) — back-compat shim. readExpertTurns delegates to
 * the canonical reviewer reader (which dual-reads via getTeammatesBlock).
 */
/**
 * @overload
 * @param {string} specPath
 ...
 *
 * @param {string} specPath
 * @param {DispatchLocator} locator
 * @param {string} findingId
 * @param {BlockerUpdate} update
 */
export function updateDispatchExpertBlocker(specPath, locator, findingId, update) {
  ...
  const blocker = target.expert_blockers.find((/** @type {JsonObject} */ b) => b.finding_id === findingId);
  if (!blocker) {
    throw new Error(
 ...
 * @param {string} taskId
 * @param {ImplementTerminal} terminal
 */
export function finalizeImplementDispatch(specPath, sliceId, taskId, terminal) {
  ...
  for (const field of /** @type {Array<'model_role' | 'model' | 'effort'>} */ (['model_role', 'model', 'effort'])) {
    if (d[field] === undefined && terminal[field] !== undefined) d[field] = terminal[field];
  }
  saveSidecar(specPath, sc);
}

/** @param {string} specPath @param {JsonObject} autopilotBlock */
 ...
 *
 * @param {string} specPath
 * @param {DependencyGraph} graphBlock
 */
...
/**
 * v0.15.0 — find sidecars with an in-flight autopilot block that has gone
 * quiet. Sidecar replay found a run abandoned at a phase start with
 * `halt_reason: null` that sat silently resumable for 11 days — with no
 * halt_reason, "abandoned" was indistinguishable from "running elsewhere".
 * Skills call this at entry and surface the results to the user.
 *
 * Scans <repoRoot>/.superpowers-codex-paired/ recursively for *.json whose
 * `autopilot` block is non-null, has `halt_reason: null`, and whose
 * last_tick_at (falling back to phase_started_at / started_at) is older
 * than `maxAgeHours`.
 *
 * @param {string} repoRoot
 * @param {{maxAgeHours?: number, now?: Date}} opts
 * @returns {Array<{sidecar_path: string, current_slice: any, current_phase: any,
 *                  last_tick_at: string|null, idle_hours: number, plan_path: any}>}
 */
/** @param {string} repoRoot @param {{maxAgeHours?: number, now?: Date}} [opts] */
export function scanStaleAutopilot(repoRoot, opts = {}) {
  const maxAgeHours = typeof opts.maxAgeHours === 'number' && Number.isFinite(opts.maxAgeHours) && opts.maxAgeHours > 0 ? opts.maxAgeHours : 24;
  const nowMs = opts.now instanceof Date ? opts.now.getTime() : Date.now();
  const base = join(repoRoot, '.superpowers-codex-paired');
  ...
  for (const ent of entries) {
    ...
    const path = join(ent.parentPath ?? ent.path, ent.name);
    let sc;
    try {
      sc = JSON.parse(readFileSync(path, 'utf8'));
    } catch {
      continue; // unreadable/corrupt files are doctor's problem, not the scan's
    }
    const ap = sc && typeof sc === 'object' ? sc.autopilot : null;
    if (!ap || typeof ap !== 'object') continue;
    if (ap.halt_reason !== null && ap.halt_reason !== undefined) continue; // halted/completed — not in-flight
    const tickIso = ap.last_tick_at || ap.phase_started_at || ap.started_at || null;
    const tickMs = tickIso ? Date.parse(tickIso) : NaN;
    if (Number.isNaN(tickMs)) continue;
    const idleHours = (nowMs - tickMs) / 3_600_000;
    if (idleHours < maxAgeHours) continue;
    out.push({
      sidecar_path: path,
      current_slice: ap.current_slice ?? null,
      current_phase: ap.current_phase ?? null,
      last_tick_at: tickIso,
      idle_hours: Math.round(idleHours * 10) / 10,
      plan_path: ap.plan_path ?? null,
    });
  }
  out.sort((a, b) => b.idle_hours - a.idle_hours);
  return out;
}
 ...
 *
 * @param {string} specPath
 * @param {GoalsBlock} goalsBlock — `block` is the verbatim
 ...
 */
/** @param {string} specPath @param {{goals: JsonObject[]}} options */
 ...
 */
/** @param {string} specPath @param {{goalId: string, planPath: string}} options */
export function markGoalShipped(specPath, { goalId, planPath }) {
  ...
  const g = sc.app_state.goals.find((/** @type {JsonObject} */ x) => x.id === goalId);
  if (!g) {
  ...
  const planRec = sc.app_state.plans.find((/** @type {JsonObject} */ p) => p.path === planPath);
  if (planRec) {
 ...
 */
/** @param {string} specPath @param {{planPath: string, started?: boolean, shipped?: boolean}} options */
export function setAppPlan(specPath, { planPath, started, shipped }) {
  ...
  let rec = sc.app_state.plans.find((/** @type {JsonObject} */ p) => p.path === planPath);
  if (!rec) {
...
export function listAudits(specPath, filter = {}) {
  ...
  return all.filter((a) => {
    ...
    return true;
  }).map((a) => ({ ...a, commands: a.commands.map((/** @type {JsonObject} */ c) => ({ ...c })) }));
}
 ...
 */
/** @param {string} specPath @param {{phase: string, round: number, side: string}} query */
export function hasAuditFor(specPath, { phase, round, side }) {
  ...
  if (!isValidAuditSide(side)) {
    throw new Error(`hasAuditFor: side must be 'claude', 'codex', or a valid member_id; got ${JSON.stringify(side)}`);
  }
  const sc = loadSidecar(specPath);
...
const MEMBER_AUDIT_SIDE_RE = /^(codex|agy):[A-Za-z0-9._-]+$/;

/** @param {unknown} side */
function isValidAuditSide(side) {
  return side === 'claude' || side === 'codex' || (typeof side === 'string' && MEMBER_AUDIT_SIDE_RE.test(side));
}
...
export function hasExecutedVerificationFor(specPath, { phase, round, side }) {
  ...
  if (!isValidAuditSide(side)) {
    throw new Error(`hasExecutedVerificationFor: side must be 'claude', 'codex', or a valid member_id; got ${JSON.stringify(side)}`);
  }
  const sc = loadSidecar(specPath);
  if (!Array.isArray(sc.audits)) return false;
...
/**
 * v0.13.0 — surface verification commands that only passed after a retry (attempts > 1 or
 * flaky:true). A flaky-passed verification still satisfies the gate (it did pass), but the SHIP
 * reviewer should see it and may challenge resting a verdict on flaky evidence. Also the substrate
 * for cross-agent flake detection: compare a test's result across the claude and codex sides.
 */
/** @param {string} specPath @param {AuditFilter} [filter] */
export function listFlakyVerifications(specPath, filter = {}) {
  const sc = loadSidecar(specPath);
  const out = [];
  for (const a of Array.isArray(sc.audits) ? sc.audits : []) {
    if (filter.phase !== undefined && a.phase !== filter.phase) continue;
    if (filter.round !== undefined && a.round !== filter.round) continue;
    for (const c of Array.isArray(a.commands) ? a.commands : []) {
      if (c.kind === 'verification' && (c.flaky === true || (Number.isInteger(c.attempts) && c.attempts > 1))) {
        out.push({ phase: a.phase, round: a.round, side: a.side, cmd: c.cmd, attempts: c.attempts ?? null, flaky: c.flaky ?? null });
      }
    }
  }
  return out;
...
export async function setCodexThreadId(
  ...
  /** @type {{role?: string, oldThreadId?: string, newThreadId?: string, reason?: string, phase?: string, round?: number, threadConfig?: JsonObject}} */
  { role = 'paired-reviewer', oldThreadId, newThreadId, reason, phase, round, threadConfig } = {},
) {
  ...
  if (threadConfig !== undefined) {
    if (!threadConfig || typeof threadConfig !== 'object' || Array.isArray(threadConfig)) {
      throw new Error('setCodexThreadId: threadConfig must be an object when present');
    }
    if (!MODEL_ROLES.includes(threadConfig.role)) {
      throw new Error(`setCodexThreadId: threadConfig.role must be one of ${MODEL_ROLES.join(', ')}`);
    }
    if ('cli' in threadConfig && threadConfig.cli !== undefined) {
      if (!VALID_CLIS.includes(threadConfig.cli)) {
        throw new Error(`setCodexThreadId: threadConfig.cli must be one of ${VALID_CLIS.join(', ')}`);
      }
    }
    for (const field of ['model', 'effort']) {
      if (typeof threadConfig[field] !== 'string' || !SAFE_TOKEN.test(threadConfig[field])) {
        throw new Error(`setCodexThreadId: threadConfig.${field} must be a non-empty safe token`);
      }
    }
  }
  return withSidecarLock(specPath, () => {
    ...
    if (threadConfig !== undefined) {
      if (!sc.thread_config || typeof sc.thread_config !== 'object') sc.thread_config = {};
      sc.thread_config[role] = { ...threadConfig, opened_at: new Date().toISOString() };
    }
 ...
 */
/** @param {string} specPath @returns {string | null} */
...
export function buildReplayContext(specPath) {
  ...
  return {
    ...
    rounds: (sc.rounds ?? []).map((/** @type {JsonObject} */ r) => ({ phase: r.phase, round: r.round, claude: r.claude, codex: r.codex })),
...
/** @param {AuditRecord} audit @returns {JsonObject} */
function normalizeAuditEntry(audit) {
  ...
  if (!isValidAuditSide(audit.side)) {
    throw new Error(`audit entry: side must be 'claude', 'codex', or a valid member_id; got ${JSON.stringify(audit.side)}`);
  }
  ...
  for (const [i, c] of audit.commands.entries()) {
    ...
    // Optional v0.13.0 flake provenance: a command that passed only after retries records how many
    // attempts it took and whether it was flaky — honest evidence the reviewer can challenge.
    if (c.attempts !== undefined && (!Number.isInteger(c.attempts) || c.attempts < 1)) {
      throw new Error(`audit entry: commands[${i}].attempts must be a positive integer when present`);
    }
    if (c.flaky !== undefined && typeof c.flaky !== 'boolean') {
      throw new Error(`audit entry: commands[${i}].flaky must be a boolean when present`);
    }
  }
  ...
  if (audit.reviewed_sha !== undefined && !/^[0-9a-f]{40}$/.test(audit.reviewed_sha)) {
    throw new Error('audit entry: reviewed_sha must be a full 40-character lowercase hex SHA');
  }
  return {
    phase: audit.phase,
    round: audit.round,
...
/**
 * Enforce audit evidence and same-commit SHIP review at both write boundaries
 * (v0.16.0 spec §6 step 5).
 */
/** @param {JsonObject} round @param {JsonObject[]} scope @param {{headSha?: string, source?: string}} [options] */
export function assertRoundShipBacked(round, scope, { headSha, source = 'appendRoundWithAudits' } = {}) {
  const isDesign = DESIGN_PHASE_RE.test(round.phase);
  const codeBearing = requiresExecutedVerification(round.phase);
  const isPanelRound = Array.isArray(round.panel);
  const panelShip = isPanelRound && verdictStatus(round.codex) === 'SHIP';
  if (!isDesign && !codeBearing && !panelShip) return;
  const roundEntries = scope.filter((a) => a.phase === round.phase && a.round === round.round);
  const shipSides = isPanelRound
    ? [
        ...(typeof round.claude === 'string' && SHIP_VERDICT_RE.test(round.claude) ? ['claude'] : []),
        ...(panelShip ? round.panel.map((/** @type {JsonObject} */ entry) => entry.member_id) : []),
      ]
    : ['claude', 'codex'].filter(
        (side) => typeof round[side] === 'string' && SHIP_VERDICT_RE.test(round[side]),
      );
  // v0.16.0 same-commit rule (spec §6 step 5). The SHA-bearing contract applies only to SHIP
  // sides in code-bearing phases; superseded audits at an earlier SHA (e.g. Claude's C0
  // verification before a fix pass) are allowed to coexist — what matters is that each SHIP
  // side has a qualifying verification AT headSha. REVISE rounds carry no SHIP obligations.
  const shaContract = codeBearing && shipSides.length > 0 && roundEntries.some((a) => a.reviewed_sha !== undefined);
  if (shaContract && (typeof headSha !== 'string' || headSha.length === 0)) {
    throw new Error(`${source}: headSha required when review audits carry reviewed_sha`);
  }

  for (const side of shipSides) {
    const entries = roundEntries.filter((a) => a.side === side);
    if (codeBearing) {
      const qualifying = entries.filter(
        (a) => Array.isArray(a.commands) && a.commands.some(isExecutedVerificationCommand),
      );
      if (qualifying.length === 0) {
        throw new Error(`${source}: ${side} SHIP in code-bearing phase ${round.phase} requires a successful kind:"verification" command (exit_code 0) that actually ran tests`);
      }
      if (shaContract) {
        if (qualifying.some((a) => a.reviewed_sha === undefined)) {
          throw new Error(`${source}: ${side} qualifying verification audit must include reviewed_sha (asymmetric omission)`);
        }
        if (!qualifying.some((a) => a.reviewed_sha === headSha)) {
          throw new Error(`${source}: ${side} has no qualifying verification audit whose reviewed_sha matches headSha ${headSha} (reviewed_sha seen: ${[...new Set(qualifying.map((a) => a.reviewed_sha))].join(', ')})`);
        }
      }
    } else if (entries.length === 0) {
      throw new Error(`${source}: ${side} SHIP in ${round.phase} has no audit entry for that side`);
    }
  }
}
 ...
 */
/** @param {string} specPath @param {{audits?: AuditRecord[], round?: Round}} [entries] @param {{headSha?: string, sequential?: boolean, budget?: boolean}} [opts] */
export async function appendRoundWithAudits(specPath, { audits, round } = {}, opts = {}) {
  ...
  return withSidecarLock(specPath, () => {
    ...
    validateRoundForAppend(sc, round, opts);                       // throws → nothing saved (v0.15.0)
    const normalized = audits.map((a) => normalizeAuditEntry(a)); // throws → nothing saved
    validatePanelRound(sc, round);                                // throws → nothing saved (v0.19.0)
    assertRoundShipBacked(round, sc.audits.concat(normalized), {
      headSha: opts.headSha,
      source: 'appendRoundWithAudits',
    });                                                           // throws → nothing saved
...
/** @param {string} sliceId @param {ImplementerRun} run */
function validateStartRun(sliceId, run) {
  ...
  for (const memberId of memberIds) {
    ...
    // v0.14.0 hybrid: optional, additive member fields. Validated only when present
    // so legacy (non-hybrid) members stay valid and byte-identical.
    if (m.owner !== undefined && m.owner !== null) {
      if (m.owner !== 'claude-ui' && m.owner !== 'codex-backend') {
        throw new Error(
          `startImplementerRun: members[${memberId}].owner must be "claude-ui" or "codex-backend" when present`
        );
      }
    }
    if (m.runtime_kind !== undefined && m.runtime_kind !== null && !nonEmptyString(m.runtime_kind)) {
      throw new Error(
        `startImplementerRun: members[${memberId}].runtime_kind must be a non-empty string when present`
      );
    }
    if (m.overlap_rationale !== undefined && m.overlap_rationale !== null && !nonEmptyString(m.overlap_rationale)) {
      throw new Error(
        `startImplementerRun: members[${memberId}].overlap_rationale must be a non-empty string when present`
      );
    }
  }
}
 ...
 * @param {string} specPath
 * @param {string} sliceId — e.g. "slice-3"
 * @param {ImplementerRun} run
 * @param {JsonObject} deps — optional dep injection (lockfile override for tests)
...
export async function startImplementerRun(specPath, sliceId, run, deps = {}) {
  ...
  return withSidecarLock(specPath, async () => {
    ...
    for (const [memberId, m] of Object.entries(run.members)) {
      ...
      // v0.14.0 hybrid: persist optional owner metadata only when supplied, so
      // legacy members stay byte-identical and older readers ignore absent keys.
      for (const key of /** @type {Array<'owner' | 'runtime_kind' | 'overlap_rationale'>} */ (['owner', 'runtime_kind', 'overlap_rationale'])) {
 ...
 *   - event.runtime_kind must equal members[member_id].runtime_kind ?? .adapter
...
export async function appendImplementerEventLocked(specPath, event, deps = {}) {
  ...
  return withSidecarLock(specPath, async () => {
    ...
    // Events carry the member's ACTUAL runtime kind. For hybrid members the
    // logical plan adapter (e.g. "claude-ui") differs from the runtime kind the
    // runner dispatched (e.g. "claude-subagent" or "claude-inline"), so validate
    // against members[id].runtime_kind when present, falling back to adapter for
    // legacy (non-hybrid) members that never carried runtime_kind (spec §6/§9).
    const expectedRuntimeKind = member.runtime_kind ?? member.adapter;
    if (expectedRuntimeKind !== event.runtime_kind) {
      throw new Error(
        `appendImplementerEventLocked: event.runtime_kind "${event.runtime_kind}" does not match ` +
          `members["${event.member_id}"].runtime_kind "${expectedRuntimeKind}"`
      );
    }
 ...
 * @param {ImplementerRunResult} result
...
/** @param {string} specPath @param {JsonObject} contention */
...
/** @param {string} sliceKey @returns {string} */
...
/** @param {string | number} sliceNumber @returns {string} */
