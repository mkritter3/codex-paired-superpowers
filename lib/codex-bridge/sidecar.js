...
export function isExecutedVerificationCommand(c) {
  ...
  if (c.selection && (c.selection.mode === 'none' || c.selection.ran === 0)) return false;
  return true;
}
...
function normalizeAuditEntry(audit) {
  ...
  for (const [i, c] of audit.commands.entries()) {
    ...
    // Optional v0.13.0 TIA selection record: must be an object with a string mode if present.
    if (c.selection !== undefined) {
      if (!c.selection || typeof c.selection !== 'object' || Array.isArray(c.selection) || typeof c.selection.mode !== 'string') {
