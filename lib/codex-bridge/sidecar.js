...
import { MODEL_ROLES, SAFE_TOKEN, roleForThread } from './models.js';
...
export function initSidecar(specPath, { feature, codexSession, model, reasoningEffort }) {
  ...
  const data = {
    ...
    thread_config: {
      'paired-reviewer': {
        role: 'planning', model, effort: reasoningEffort, opened_at: openedAt,
      },
    },
    rounds: [],
    open_contentions: [],
...
export function getThreadConfig(sidecarData, key = 'paired-reviewer') {
  ...
  return { role: roleForThread(key), model: null, effort: null, legacy: true };
}
