 ...
 *
 * @param {object} sc — loaded sidecar (rounds may be absent on legacy files)
 * @param {object} round
...
export function validateRoundForAppend(sc, round, opts = {}) {
  ...
  const existing = (Array.isArray(sc.rounds) ? sc.rounds : [])
    .filter((r) => r && typeof r === 'object' && r.phase === round.phase);
  const maxExisting = existing.reduce(
    (m, r) => (Number.isInteger(r.round) ? Math.max(m, r.round) : m), 0,
  );
  if (opts.sequential) {
 ...
 * @param {object} block — the §9 hybrid status block (owners + contract sync state)
 */
...
export function scanStaleAutopilot(repoRoot, opts = {}) {
  const maxAgeHours = Number.isFinite(opts.maxAgeHours) && opts.maxAgeHours > 0 ? opts.maxAgeHours : 24;
