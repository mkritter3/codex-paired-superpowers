...
export function appendAuditLog(specPath, audit) {
  if (!audit || typeof audit !== 'object' || Array.isArray(audit)) {
    throw new Error('appendAuditLog: audit must be a plain object');
  }
  if (typeof audit.phase !== 'string' || audit.phase.length === 0) {
    throw new Error('appendAuditLog: audit.phase must be a non-empty string');
  }
  if (!Number.isInteger(audit.round) || audit.round < 1) {
    throw new Error('appendAuditLog: audit.round must be a positive integer');
  }
  if (audit.side !== 'claude' && audit.side !== 'codex') {
    throw new Error(`appendAuditLog: audit.side must be 'claude' or 'codex'; got ${JSON.stringify(audit.side)}`);
  }
  if (!Array.isArray(audit.commands) || audit.commands.length === 0) {
    throw new Error('appendAuditLog: audit.commands must be a non-empty array');
  }
  for (const [i, c] of audit.commands.entries()) {
    if (!c || typeof c !== 'object' || Array.isArray(c)) {
      throw new Error(`appendAuditLog: audit.commands[${i}] must be a plain object`);
    }
    if (typeof c.cmd !== 'string' || c.cmd.length === 0) {
      throw new Error(`appendAuditLog: audit.commands[${i}].cmd must be a non-empty string`);
    }
    if (typeof c.summary !== 'string' || c.summary.length === 0) {
      throw new Error(`appendAuditLog: audit.commands[${i}].summary must be a non-empty string`);
    }
    // v0.13.0 — kind is required for new audit entries; exit_code required for verification.
    if (!VALID_AUDIT_KINDS.has(c.kind)) {
      throw new Error(`appendAuditLog: audit.commands[${i}].kind must be one of inspection|verification|other`);
    }
    if (c.kind === 'verification' && !Number.isInteger(c.exit_code)) {
      throw new Error(`appendAuditLog: audit.commands[${i}].exit_code must be an integer for kind "verification"`);
    }
  }
  if (audit.verdict_basis !== undefined && (typeof audit.verdict_basis !== 'string' || audit.verdict_basis.length === 0)) {
    throw new Error('appendAuditLog: audit.verdict_basis must be a non-empty string when present');
  }
