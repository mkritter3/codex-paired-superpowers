 ...
 */
export function assertShipAuditsForAppend(sc, round) {
  const isDesign = DESIGN_PHASE_RE.test(round.phase);
  const isCodeBearing = requiresExecutedVerification(round.phase);
  if (!isDesign && !isCodeBearing) return;
  const audits = Array.isArray(sc.audits) ? sc.audits : [];
  for (const side of ['claude', 'codex']) {
    const verdict = round[side];
    if (typeof verdict !== 'string' || !SHIP_VERDICT_RE.test(verdict)) continue;
    const entries = audits.filter((a) => a.phase === round.phase && a.round === round.round && a.side === side);
    if (isCodeBearing) {
      const ok = entries.some((a) => Array.isArray(a.commands) && a.commands.some(isExecutedVerificationCommand));
      if (!ok) {
        throw new Error(
          `appendRound: ${side} SHIP in code-bearing phase ${round.phase} round ${round.round} requires a successful ` +
          'kind:"verification" command (exit_code 0) that actually ran tests. Record audits and the round atomically ' +
          'with sidecar-append-round-with-audits, or append the missing side via sidecar-append-audit first. ' +
          'If you did not verify, do NOT fabricate evidence — emit REVISE and ask the other side to audit next round.',
        );
      }
    } else if (entries.length === 0) {
      throw new Error(
        `appendRound: ${side} SHIP in ${round.phase} round ${round.round} has no audit entry for that side. ` +
        'Record audits and the round atomically with sidecar-append-round-with-audits (include "kind"), or append ' +
        'the missing side via sidecar-append-audit first. If you did not verify, emit REVISE instead.',
      );
    }
  }
}
...
export function appendRound(specPath, round, opts = {}) {
  ...
  if (opts.enforceShipAudits) assertShipAuditsForAppend(sc, round);
