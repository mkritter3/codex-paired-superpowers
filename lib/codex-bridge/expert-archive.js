// v0.8.0 expert-archive — halt-reason-driven archival policy for expert mailboxes.
//
...
//   - Unknown halt reasons → throw ExpertArchiveError code unknown-halt-reason
...
//
// The orchestrator (per skills/autopilot/SKILL.md §Mailbox archival policy)
// iterates `expert_teammates.selected[]` at terminal-state halt and calls
// this once per active expert, then persists the returned record alongside
// the expert's status transition in the sidecar.
...
export const HALT_REASONS_PRESERVE = new Set([
  ...
  // These are formalized by halt-envelope.js; extend here so expert-archive
...
/**
 * Apply the halt-reason archival policy for a single expert identity.
 *
 * @param {{id:string}} identity — ExpertIdentity (only `id` is required).
...
export async function archive(identity, haltReason, deps = {}) {
  ...
  if (!identity || typeof identity.id !== 'string' || identity.id.length === 0) {
    throw new ExpertArchiveError(
      'invalid-identity',
      `archive requires an ExpertIdentity with non-empty id; got ${JSON.stringify(identity)}`
    );
  }
    ...
    try {
      ...
      try {
        d.writeBreadcrumb(
          d.repoRoot,
          identity.id,
          `expert-archive archiveAndReset failed: ${err.code || err.message}`
        );
      } catch {
