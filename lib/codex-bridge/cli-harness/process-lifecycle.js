...
export async function runChildWithLifecycle(params) {
  // Public contract: `params.command` is required (ProcessLifecycleParams). The implementation
  // types its own view honestly as Partial so the missing-field path is visible: a missing
  // command reaches `spawn` as '' and fails inside the try below exactly like any bad command.
  const {
    command,
    args = [],
    cwd,
  ...
  } = /** @type {Partial<ProcessLifecycleParams>} */ (params ?? {});
