...
export async function runChildWithLifecycle(params = {}) {
  ...
  try {
    ...
    // `command` is `string | undefined` in the implementation's honest view. The cast is at the
    // I/O boundary where Node itself validates the same value: `spawn` throws ERR_INVALID_ARG_TYPE
    // for any non-string synchronously, inside this try, so the native error (message and all)
    // is exactly what the spawn-failed result carries — no emulation.
    child = spawn(/** @type {string} */ (command), args, spawnOpts);
