...
export async function dispatch(systemPrompt, userPrompt, options = {}) {
  ...
  try {
    ...
    // Implementer mode: set the working directory for the spawn.
    // Reviewer mode: leave cwd undefined (inherits current process cwd).
    if (spawnCwd !== undefined) {
      spawnOpts.cwd = spawnCwd;
    }
    child = spawn(command, args, spawnOpts);
    if (typeof child.pid === 'number' && typeof options.onSpawn === 'function') {
  ...
  try { options.signal?.removeEventListener?.('abort', onExternalAbort); } catch { /* noop */ }
