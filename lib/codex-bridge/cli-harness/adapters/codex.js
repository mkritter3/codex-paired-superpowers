...
/**
 * @typedef {object} CodexDispatchOptions
 ...
 * @property {{halt: string, terminal: boolean, resume_hint: string}} [haltEnvelope]
 */
...
/**
 * Dispatch through the Codex CLI (robustness spec §1 owner contract).
 * @param {string} systemPrompt
 * @param {string} userPrompt
 * @param {CodexDispatchOptions} [options]
...
export async function dispatch(systemPrompt, userPrompt, options = {}) {
  ...
  if (Array.isArray(options.args)) {
    ...
    args = ['exec', '--sandbox', 'workspace-write', '-C', /** @type {string} */ (options.cwd)];
  } else {
    args = ['--json'];
  }
  if (hasModelOptions) {
    args = [
      ...
      '-c',
      `model_reasoning_effort=${/** @type {string} */ (options.reasoningEffort)}`,
    ];
  }
...
/**
 * @param {string} stdout
 * @returns {{ok: true, responseText: string, eventCount: number, unknownEventCount: number} | {ok: false, error: string}}
 */
