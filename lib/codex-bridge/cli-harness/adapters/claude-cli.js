 ...
 * Accepts NDJSON lines.
 * Returns { ok, responseText, eventCount, unknownEventCount, recognizedEventCount }
 * or { ok: false, error: string }.
 *
 * recognizedEventCount counts events whose type was actually handled
 * (text/result/assistant_message or content_block_delta with text_delta).
 * If ok is true AND recognizedEventCount === 0 AND eventCount > 0, the caller
 * should treat it as protocol-unsupported (schema drift).
 */
function parseStreamJson(stdout) {
  ...
  if (allLinesParsed) {
    return { ok: true, responseText, eventCount, unknownEventCount, recognizedEventCount };
  }
...
async function dispatchAsync(systemPrompt, userPrompt, options) {
  ...
  // EPIPE from a child that exited before (or while) we write reaches the stream as an
  // asynchronous 'error' event, NOT as a synchronous throw from end() — the try/catch below cannot
  // see it, and an unhandled 'error' on stdin takes the process down (observed in CI: `Error: write
  // EPIPE` out of dispatchAsync). Same guard as cli-harness/process-lifecycle.js.
  child.stdin.on('error', () => { /* child exited or was killed before stdin was consumed */ });
  try {
    child.stdin.end(stdinPayload);
  } catch {
    // A synchronous throw (e.g. already-destroyed stream); the async path is guarded above.
  }
  ...
  if (timedOut || exitCode !== 0) {
    ...
    if (!parsed.ok) {
    ...
    } else if (parsed.eventCount > 0 && parsed.recognizedEventCount === 0) {
      // All events were valid JSON but had unknown types — schema has drifted.
      // Classify as protocol-unsupported rather than silently succeeding.
      const envelope = wrapAsHaltEnvelope('claude-cli-protocol-unsupported', {
        sliceId: options.sliceId,
        phase: 'implement',
      });
      const result = {
        ...
        adapterMeta: {
          ...
          unknownEventCount: parsed.unknownEventCount,
          recognizedEventCount: parsed.recognizedEventCount,
          schemaDrift: true,
        },
        duration_ms,
        haltEnvelope: {
      ...
      adapterMeta.recognizedEventCount = parsed.recognizedEventCount;
    }
  }
