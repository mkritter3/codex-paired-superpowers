...
export async function getAdapter(name) {
  ...
  // Dispatchable runtime kinds: undefined/null (legacy), 'cli-harness', 'claude-cli'.
  const DISPATCHABLE_RUNTIME_KINDS = new Set(['cli-harness', 'claude-cli']);
  if (config.runtime_kind && !DISPATCHABLE_RUNTIME_KINDS.has(config.runtime_kind)) {
    throw new RegistryError(
