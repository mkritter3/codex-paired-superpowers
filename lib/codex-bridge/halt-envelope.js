 ...
 *
 * Strict fail-closed: anything that is not an object with `terminal === false`
 * is treated as terminal. Malformed envelopes (missing terminal, non-boolean
 * terminal, non-object, null/undefined) MUST stop ralph-loop — re-firing on a
 * malformed result would loop indefinitely (Codex round-1 slice-7b finding #3).
 *
 * @param {{terminal?: boolean} | null | undefined} envelope
...
export function isTerminalHalt(envelope) {
  ...
  // Only the explicit `terminal === false` form means "transient, safe to retry".
  // Any other shape (missing terminal, non-boolean terminal, missing
  // resume_hint) is treated as terminal.
  if (envelope.terminal !== false && envelope.terminal !== true) return true;
  if (envelope.terminal === true) return true;
  // terminal === false: validate the rest of the contract before allowing retry.
