...
/** @template T @param {any} fn @param {string} checkoutPath @returns {Promise<T> | T} */
function invokeReviewCallback(fn, checkoutPath) {
  return fn(checkoutPath);
}
 ...
 * @param {((checkoutPath: string) => Promise<T> | T)} [fn]
 * @returns {Promise<T>}
 */
export async function withReviewCheckout(repoRoot, { sha = 'HEAD', overlayPaths = [] } = {}, fn) {
  ...
  try {
    ...
    return await invokeReviewCallback(fn, checkoutPath);
  } finally {
