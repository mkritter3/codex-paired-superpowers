...
import { writeOwnershipMarker } from './checkout-markers.js';

/** @typedef {{ok: false, halt: {reason: string, detail: string}}} HaltResult */
/** @typedef {{ok: true, stdout: string} | {ok: false, stderr: string, status: number | null}} GitResult */
/** @typedef {{path?: string, branch?: string}} WorktreeRecord */

// ── helpers ───────────────────────────────────────────────────────────────────

/** @param {string} reason @param {string} detail @returns {HaltResult} */
 ...
 */
/** @param {string[]} args @param {{cwd?: string}} [opts] @returns {GitResult} */
 ...
 */
/** @param {string} repoRoot @param {string} sliceId */
 ...
 */
/** @param {string} repoRoot @returns {WorktreeRecord[]} */
...
export function create(repoRoot, sliceId, sliceStartSha) {
  ...
  try {
    ...
    runGit(['worktree', 'remove', '--force', worktreePath], { cwd: repoRoot });
    runGit(['branch', '-D', branch], { cwd: repoRoot });
    return halt('worktree-create-failed', `failed to write ownership marker for ${worktreePath}: ${error.message}`);
  }
...
/**
 * Create a throwaway detached worktree at the given SHA under OS tmpdir.
 *
 * Spec: docs/specs/2026-09-17-v0.17.0-antigravity-transport-design.md §4
 *
 * @param {string} repoRoot
 * @param {string} sha
 * @returns {{ ok: true, worktreePath: string } | { ok: false, halt: { reason: string, detail: string } }}
 */
export function createDetached(repoRoot, sha) {
  const worktreePath = mkdtempSync(join(tmpdir(), 'cps-review-'));
  const r = runGit(
    ['worktree', 'add', '--detach', worktreePath, sha],
    { cwd: repoRoot },
  );
  if (!r.ok) {
    try {
    ...
    return halt(
      'worktree-create-failed',
      `git worktree add --detach ${worktreePath} ${sha} failed: ${r.stderr.trim()}`,
    );
  }
...
/**
 * Run a callback inside a throwaway detached checkout with overlaid uncommitted files.
 *
 * Spec: docs/specs/2026-09-17-v0.17.0-antigravity-transport-design.md §4
 *
 * @template T
 * @param {string} repoRoot
 * @param {{ sha?: string, overlayPaths?: string[] }} options
 * @param {(checkoutDir: string) => Promise<T> | T} fn
 * @returns {Promise<T>}
 */
export async function withReviewCheckout(repoRoot, { sha = 'HEAD', overlayPaths = [] } = {}, fn) {
  for (const p of overlayPaths) {
    if (typeof p !== 'string' || isAbsolute(p)) {
      ...
      const err = new Error(`overlay path must be a relative string: ${JSON.stringify(p)}`);
      err.code = 'review-overlay-invalid';
      throw err;
    }
    const parts = p.split(/[\\/]/);
    // Git metadata is never an overlay: copying a linked checkout's `.git` pointer file would
    // repoint the review checkout at another checkout's admin directory (and its markers).
    if (parts.some((part) => part.toLowerCase() === '.git')) {
      /** @type {Error & {code?: string}} */
      const err = new Error(`overlay path must not include git metadata: ${p}`);
      err.code = 'review-overlay-invalid';
      throw err;
    }
    if (parts.includes('..')) {
      /** @type {Error & {code?: string}} */
      const err = new Error(`overlay path must not contain '..': ${p}`);
      err.code = 'review-overlay-invalid';
      throw err;
    }
    const fullSrc = join(repoRoot, p);
    if (!existsSync(fullSrc)) {
      /** @type {Error & {code?: string}} */
      const err = new Error(`overlay path does not exist in repo: ${p}`);
      err.code = 'review-overlay-invalid';
      throw err;
    }
  }

  const created = createDetached(repoRoot, sha);
  if (!created.ok) {
    /** @type {Error & {code?: string}} */
    const err = new Error(`createDetached failed (${created.halt.reason}): ${created.halt.detail}`);
    err.code = created.halt.reason;
    throw err;
  }

  const checkoutPath = created.worktreePath;
  try {
    // Ownership is stamped before overlays touch the tree, while `.git` is still the pointer
    // createDetached wrote.
    const resolvedBase = runGit(['rev-parse', 'HEAD'], { cwd: checkoutPath });
    if (!resolvedBase.ok) {
      /** @type {Error & {code?: string}} */
      const err = new Error(`failed to resolve review checkout base: ${resolvedBase.stderr.trim()}`);
      err.code = 'worktree-create-failed';
      throw err;
    }
    writeOwnershipMarker(checkoutPath, {
      kind: 'review',
      run_id: `review-${randomUUID()}`,
      base: resolvedBase.stdout.trim(),
      overlays: overlayPaths,
    });
    for (const p of overlayPaths) {
      const fullSrc = join(repoRoot, p);
      const fullDest = join(checkoutPath, p);
      mkdirSync(dirname(fullDest), { recursive: true });
      copyFileSync(fullSrc, fullDest);
    }
    return await fn(checkoutPath);
  } finally {
    // Remove ONLY this review's checkout. `git worktree remove --force <path>` drops its own
    // registry entry, so no prune is needed. A repository-wide `git worktree prune` used to follow
    // here, which on every finished review dropped every OTHER stale entry in the repository —
    // including a detached HEAD's last reference to a commit that existed nowhere else (v0.19.0 spec
    // §6; regression tests in tests/codex-bridge/worktree.test.js).
    runGit(['worktree', 'remove', '--force', checkoutPath], { cwd: repoRoot });
    try {
