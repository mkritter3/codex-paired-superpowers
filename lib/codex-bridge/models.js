...
export class ModelsConfigError extends Error {
  constructor(detail, code = 'models-config-malformed') {
    super(detail);
...
export function resolveModelRoles({ repoRoot = process.cwd(), env = process.env } = {}) {
  ...
  if (loaded && !loaded.ok) {
    throw new ModelsConfigError(loaded.error.detail, loaded.error.code);
  }
  ...
  if (projectModels !== null && projectModels !== undefined) {
    ...
    if (error) throw new ModelsConfigError(error.detail, error.code);
