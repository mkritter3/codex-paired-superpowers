...
/** @typedef {import('./types.js').ModelRole} ModelRole */
/** @typedef {import('./types.js').RoleConfig} RoleConfig */
...
/** @type {Readonly<Record<ModelRole, Readonly<RoleConfig>>>} */
...
/** @param {string} value @returns {value is ModelRole} */
...
/**
 * How agy runs headless. `auto` (the default) is `--sandbox --dangerously-skip-permissions`: the
 * sandbox confines writes to the working folder, and nothing waits for an approval nobody can
 * give. `accept-edits` drops the skip flag, so every command must match the user's own agy
 * allow-list (anything else is refused and ends the turn).
 * @param {NodeJS.ProcessEnv} [env]
 * @returns {'auto' | 'accept-edits'}
 */
export function agyPermissionMode(env = process.env) {
  const raw = env.CODEX_PAIRED_AGY_PERMISSIONS;
  if (raw === undefined || raw === '' || raw === 'auto') return 'auto';
  if (raw === 'accept-edits') return 'accept-edits';
  throw new ModelsConfigError(`CODEX_PAIRED_AGY_PERMISSIONS must be "auto" or "accept-edits"; got ${JSON.stringify(raw)}`);
}
...
/** @param {unknown} value @param {string} path @returns {MalformedResult | null} */
...
export function resolveModelRoles({ repoRoot = process.cwd(), env = process.env } = {}) {
  /** @type {Record<string, {cli: string, model: string, effort: string}>} */
  const roles = {};
 ...
 * @param {string} role
 * @param {Record<string, {cli: string, model: string, effort: string}>} roles
 * @returns {{cli: string, model: string, effort: string}}
 */
 ...
 * @param {string} role
 * @param {Record<string, {cli: string, model: string, effort: string}>} roles
 * @returns {{model: string, config: {model_reasoning_effort: string}}}
 */
 ...
 * @param {string} phase
 * @returns {{role: ModelRole, warning: string | null}}
 */
 ...
 * @param {string} phase
 * @param {{repoRoot?: string, env?: Record<string, string | undefined>, roles?: Record<string, {cli: string, model: string, effort: string}>}} [options]
 */
