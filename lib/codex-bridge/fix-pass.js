...
function restoreSnapshot(repoRoot, snapshot) {
  for (const { relativePath, copy } of snapshot.entries) {
    const abs = resolve(repoRoot, relativePath);
    mkdirSync(dirname(abs), { recursive: true });
    copyFileSync(copy, abs);
  }
}
...
export async function runFixPass({
  ...
  try {
    return await runFixPassBody({
      specPath, sliceId, repoRoot, pass, execFn, reset, reconcile,
...
async function runFixPassBody({
  specPath, sliceId, repoRoot, execFn, reset, reconcile,
  fixStartSha, untrackedBefore, preserved, checkpointIndex,
}) {
