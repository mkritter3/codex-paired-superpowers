...
import { copyFileSync, existsSync, lstatSync, mkdirSync, mkdtempSync, rmSync, rmdirSync, unlinkSync } from 'node:fs';
...
function removeNewUntracked(repoRoot, before) {
  ...
  for (const relativePath of created) {
    ...
    // lstat, not existsSync: existsSync follows symlinks, so a DANGLING link the pass created
    // would read as absent and survive the rollback. unlinkSync removes the link itself on every
    // Node major (on Node 23 rmSync resolves the path first and is a silent no-op on such a link).
    let entry;
    try { entry = lstatSync(target); } catch { continue; }
...
function snapshotUntracked(repoRoot, paths, deps = {}) {
  const copy = deps.copyFile ?? copyFileSync; // same test seam as restoreSnapshot
  ...
  try {
    for (const relativePath of paths) {
      ...
      const target = join(dir, `${entries.length}.bin`);
      copy(abs, target);
      entries.push({ relativePath, copy: target });
    }
  } catch (error) {
    // A capture failure (e.g. ENOSPC on a later copy) must not strand partial backups: nothing
    // has launched yet and no checkpoint names this directory (Codex review round 5).
...
export async function runFixPass({
  ...
  const preserved = snapshotUntracked(repoRoot, untrackedBefore, _deps); // BEFORE the pass runs
  let checkpointIndex;
  try {
