...
// it outright. So the contents are captured BEFORE the pass runs (Codex review round 2), not
// after. Files larger than SNAPSHOT_MAX_BYTES are not buffered; they are recorded under
// `preserve_skipped` on the checkpoint so the omission is visible rather than silent.
...
function snapshotUntracked(repoRoot, paths) {
  ...
  for (const relativePath of paths) {
    ...
    if (size > SNAPSHOT_MAX_BYTES) { skipped.push(relativePath); continue; }
    kept.push({ relativePath, content: readFileSync(abs) });
  }
  return { kept, skipped };
}

function restoreSnapshot(repoRoot, snapshot) {
  for (const { relativePath, content } of snapshot.kept) {
    const abs = resolve(repoRoot, relativePath);
...
export async function runFixPass({
  ...
  const checkpointIndex = appendFixPass(specPath, sliceId, {
    ...
    ...(preserved.skipped.length ? { preserve_skipped: preserved.skipped } : {}),
  });
