...
//   - no `status: 'available'` entry has `resolved_path: null` (round-2 fix:
//     session-derived availability — e.g. claude-task gated on a Claude
//     Code env marker — is environment-scoped and MUST NOT be trusted
//     across processes/environments. Detector must re-probe.)
//
...
export function isCacheFresh(cachePayload, nowMs, ttlMs = DEFAULT_TTL_MS, options = {}) {
  ...
  for (const entry of Object.values(entries)) {
    ...
    if (entry.status === 'available') {
      // Round-2 critical fix: session-scoped availability (status=available
      // with resolved_path=null) MUST NOT be reused from cache. The only
      // way a prober reports available without a resolved binary path is
      // through an environment-derived signal (e.g. claude-task sees a
      // Claude Code session env marker). That signal does not transit
      // across processes — a later session B reading the cache could be
      // running outside Claude Code, but the cache would still claim
      // claude is available, routing dispatch to a CLI that can't run.
      // Fail-closed: re-probe.
      if (entry.resolved_path === null || entry.resolved_path === undefined) {
        return false;
      }
      try {
