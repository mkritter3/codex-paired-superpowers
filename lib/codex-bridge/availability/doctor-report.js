...
/** @param {{cli: string, variant?: string | null}} entry */
...
function statusLabel(status) {
  switch (status) {
    ...
    // v0.15.0 — binary present and versioned, but the auth probe failed.
    // Dispatching to it would park on an interactive login prompt.
    case 'unauthenticated': return 'NOT LOGGED IN';
    default:                return 'UNKNOWN';
  }
}

/** @param {{name: string, status: string, version?: string, resolved_path?: string, error?: string}} payload */
...
/**
 * @param {string} role
 * @param {{preference: Array<{cli: string, variant?: string | null}>}} recEntry
...
async function main() {
  ...
  if (opts.json) {
    ...
    for (const role of sortedRolesJson) {
      ...
      rolesOut.push({
        role,
        ladder: rec.preference.map((/** @type {{cli: string, variant?: string | null}} */ p) => ({ cli: p.cli, variant: p.variant || null })),
        chosen: first
