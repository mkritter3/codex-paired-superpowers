...
export async function dispatchPanel(role, request, dispatchFns, deps = {}) {
  for (const [field, value] of [
    ['panel_min_size', deps.panel_min_size],
    ['panel_max_size', deps.panel_max_size],
    ['hard_min_size', deps.hard_min_size],
  ]) {
    if (value !== undefined && (!Number.isInteger(value) || value < 1)) {
      throw new PanelDispatchError('panel-config-invalid', `${field} must be a positive integer`);
    }
  }
  if (deps.member_timeout_ms !== undefined
    && (!Number.isFinite(deps.member_timeout_ms) || deps.member_timeout_ms < 0)) {
    throw new PanelDispatchError('panel-config-invalid', 'member_timeout_ms must be a non-negative number');
  }
  const rawMin =
  ...
  const hardMinSize =
    Number.isInteger(deps.hard_min_size) && deps.hard_min_size >= 1
      ? deps.hard_min_size
      : HARD_FLOOR;

  // Expert panels retain the historical hard floor of two. The review-panel
  // caller explicitly lowers `hard_min_size` to one for configured singleton
  // rosters; its minimum remains max(hard_min_size, panel_min_size).
  const effectiveMin = Math.max(hardMinSize, rawMin);
  ...
  if (effectiveMax < effectiveMin) {
    throw new PanelDispatchError(
      ...
      `panel_max_size (${effectiveMax}) < effective panel_min_size (${effectiveMin}); ` +
        `hard floor is ${hardMinSize}`,
      {
        effectiveMin,
        effectiveMax,
        hardFloor: hardMinSize,
        rawMin,
        rawMax,
      },
    );
  ...
  if (allMembers.length < effectiveMin) {
    throw new PanelDispatchError(
      ...
      { available: allMembers.length, panel_min_size: effectiveMin, hardFloor: hardMinSize },
    );
  }
