 ...
 * This only RUNS the turn: the caller stores the conversation with storePanelMemberConversation
 * once the member's verdict is accepted, so a failed turn never stores one.
 *
 * @param {PanelMemberOptions} options
 * @param {ReviewerDeps} [deps]
 * @returns {Promise<ReviewerTurnResult & {previousThreadId: string | undefined, resumed: boolean, recovered: boolean}>}
 */
export async function openPanelMemberThread(options, deps = {}) {
  ...
  const { sidecarKey } = normalizeRole(role);
  ...
  // Real agy (observed 2026-09-18) does not fail on a lost id: it warns `conversation "<id>" not
  // found` on stderr, starts a new conversation and reports SUCCESS. That new conversation got
  // this round's full prompt, replay included, so a changed id is a recovery too.
  if (storedId && outcome.ok && typeof threadId === 'string' && threadId !== storedId) recovered = true;
  return {
    threadId,
    previousThreadId: storedId,
...
/**
 * Store a panel member's conversation after its turn was accepted (verdict parsed, version
 * matched). No-op when the conversation did not change.
 *
 * @param {Pick<PanelMemberOptions, 'role' | 'specPath' | 'repoRoot' | 'member_id' | 'model'>} options
 * @param {{threadId?: string, previousThreadId?: string}} turn
 */
export async function storePanelMemberConversation(options, turn) {
  const { role, specPath, repoRoot, member_id: memberId, model } = options;
  const { modelRole, sidecarKey } = normalizeRole(role);
  const { threadId, previousThreadId } = turn ?? {};
  if (typeof threadId !== 'string' || threadId.length === 0 || threadId === previousThreadId) return;
  const fullSpec = isAbsolute(specPath) ? specPath : join(repoRoot || process.cwd(), specPath);
  await setCodexThreadId(fullSpec, {
    role: `${sidecarKey}:${memberId}`,
    oldThreadId: previousThreadId,
    newThreadId: threadId,
    reason: previousThreadId ? 'panel-member-recovered' : 'panel-member-open',
