...
export function parseSliceHighStakes(sliceSection) {
  ...
  //   **high_stakes: true**
  //   ```
  // does not silently flip the parser. Track fence state per CommonMark
  // section 4.5: opening fence is a run of >=3 backticks OR >=3 tildes;
  // the closing fence MUST be the same marker character AND have a
  // length >= the opening run. That means a 4-backtick fence can contain
  // a 3-backtick fence (the inner ``` does NOT close the outer 4-tick).
  //
  // Round-1 review critique: prior version stored only marker style, not
  // length, so a 4-tick outer fence containing a 3-tick example would
  // close early and expose **high_stakes: true** in the inner block.
  let inFence = false;
  let fenceMarkerChar = null;   // '`' or '~'
  let fenceMarkerLen = 0;       // length of opening run (>=3)
  for (const raw of lines) {
    const t = raw.trim();
    if (!inFence) {
      // Detect an opening fence: leading run of `` ` `` or `~` of length >=3.
      const m = /^(`{3,}|~{3,})/.exec(t);
      if (m) {
        inFence = true;
        fenceMarkerChar = m[1][0];
        fenceMarkerLen = m[1].length;
        continue;
      }
    } else {
      // A line closes the fence only if it is a run of the SAME marker
      // character with length >= the opening run, and contains nothing
      // else after trim (CommonMark 4.5: closing fence may be followed
      // only by spaces).
      const closeRe = fenceMarkerChar === '`'
        ? /^(`{3,})\s*$/
        : /^(~{3,})\s*$/;
      const cm = closeRe.exec(t);
      if (cm && cm[1].length >= fenceMarkerLen) {
        inFence = false;
