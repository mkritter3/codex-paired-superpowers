...
import { assertPanelRouteSupported } from '../review-panel-run.js';
...
/**
 * Default single-implementer route. The single-implementer path is skill-level
 * orchestration — only the orchestrator (Claude) can dispatch the implementing
 * subagent via the Task tool (see skills/subagent-driven-development Step A).
 * `runSplit` is a pure router and must not acquire subagent/Task I/O, so for a
 * `single` work item it returns a normalized DIRECTIVE naming the work instead
 * of performing it. The skill orchestrator reads `outcome.kind ===
 * 'dispatch-single'` and runs its existing single-implementer dispatch (Step A
 * for interactive, the single-implementer phase for autopilot), using `driver`
 * to pick which.
 *
 * Tests (and any future programmatic single runner) override this via
 * `deps.runSingle`.
 *
 * @param {{driver: string, specPath: string, sliceId: string, repoRoot: string}} args
 * @returns {{kind: 'dispatch-single', driver: string, specPath: string, sliceId: string, repoRoot: string}}
 */
...
export async function runSplit({ driver, planPath, specPath, workItem, repoRoot, deps = {} }) {
  ...
  // Configured multi-member review panels cannot yet be represented by the
  // two-disjoint or hybrid post-implementation gates. Keep this after split
  // normalization, but before resolving or invoking any runner.
  (deps.assertPanelRouteSupported ?? assertPanelRouteSupported)({
    repoRoot,
  ...
  switch (normalized.split) {
    case 'single':
      ...
      // The default _runSingle returns a sync directive; an injected runner may be
