...
import { redactSecretFields } from './implementer/secret-redaction.js';
...
export function writeBreadcrumb(repoRoot, sliceId, errMsg) {
  try {
    ...
    // Redact any secret patterns from the error message before persisting.
    const safeErrMsg = typeof errMsg === 'string'
      ? redactSecretFields(errMsg)
      : errMsg;
