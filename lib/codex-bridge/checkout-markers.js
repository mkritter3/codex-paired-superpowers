...
function readMarker(path, validate) {
  // lstat, not stat: only a missing directory entry is `absent`. A symlink — valid target or
  // dangling — or any non-regular entry is `invalid`, which classifies fail-closed.
  try {
    if (!lstatSync(path).isFile()) return { state: 'invalid', value: null };
  ...
  try {
    ...
    // The entry existed at lstat time; a read failure now is never evidence of absence.
    return { state: 'invalid', value: null };
  }
