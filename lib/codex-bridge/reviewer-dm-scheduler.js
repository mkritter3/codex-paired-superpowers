// Plan 3 (reviewer naming migration) — reviewer-dm-scheduler: peer DM drain
// loop for the reviewer runtime. Canonical home for the body that previously
// lived in expert-dm-scheduler.js (now a shim).
//
// Scheduler detects unread DMs across active reviewers and delegates each
// turn to runTurn, which owns the full read→parse→mark-read cycle. Scheduler
// is sidecar-READ-ONLY (no append); runTurn owns appends.
//
// Loop bounds: maxRespawnsPerExpert (default 2), maxTotalTurns (default 8).
// On cap-exceeded: returns halt: "expert-peer-dm-drain-cap-exceeded" (literal
// preserved — these halt codes are honored by HALT_REASONS_PRESERVE).
...
export async function drainPeerDMs(activeExperts, deps, opts = {}) {
  ...
  while (true) {
    ...
    // v0.8.1 — peer-DM enqueue failure halt (primary detector).
    // runTurn returns peer_dm_summary on success paths. If any outbound
    // peer-DM enqueue failed, the orchestrator must surface to user /
    // debug — caps and convergence are independent concerns. Mailboxes
    // are preserved (per HALT_REASONS_PRESERVE) so resume can inspect
    // the failed targets via sidecar peer_messages_failed.
    //
    // We check the summary directly rather than scanning the sidecar so the
    // halt is observed even when appendExpertTurn was best-effort-lost
    // (the runtime intentionally swallows sidecar write errors via
    // safeAppendTurn). Sidecar scanning in autopilot B.5.5 remains a
    // secondary safety net.
    if (
      result &&
      result.peer_dm_summary &&
      typeof result.peer_dm_summary.failed === 'number' &&
      result.peer_dm_summary.failed > 0
    ) {
      return { turns, halt: 'expert-peer-dm-enqueue-failed' };
    }
  }
}
