...
function serialize(v) {
  // Preserve critique on SHIP too — v0.4 validation rubric rides in critique
  // bullets even on SHIP (rubric coverage is auditable evidence). Dropping it
  // would lose that history from the sidecar.
  if (v.status === 'SHIP') {
    if (!v.critique || v.critique.length === 0) return 'SHIP';
    return `SHIP (${v.critique.join('; ')})`;
  }
