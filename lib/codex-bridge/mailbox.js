...
// expert-* (one-window contract — expert-* acceptance is NOT removed).
const RECIPIENT_RE = /^(orchestrator|slice-\d+|reviewer-[a-z][a-z0-9-]{0,47}|expert-[a-z][a-z0-9-]{0,47}|impl-[a-z0-9][a-z0-9-]{0,60})$/;
...
function validateRecipient(value, fieldName) {
  if (typeof value !== 'string' || !RECIPIENT_RE.test(value)) {
    throw new MailboxError(
      'mailbox-recipient-malformed',
      `${fieldName} must match /^(orchestrator|slice-\\d+|reviewer-[a-z][a-z0-9-]{0,47}|expert-[a-z][a-z0-9-]{0,47}|impl-[a-z0-9][a-z0-9-]{0,60})$/; got ${JSON.stringify(value)}`
    );
  }
}
