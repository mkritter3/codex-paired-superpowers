...
const RECIPIENT_RE = /^(orchestrator|slice-\d+|expert-[a-z][a-z0-9-]{0,47})$/;
...
function validateRecipient(value, fieldName) {
  if (typeof value !== 'string' || !RECIPIENT_RE.test(value)) {
    throw new MailboxError(
      'mailbox-recipient-malformed',
      `${fieldName} must match /^(orchestrator|slice-\\d+|expert-[a-z][a-z0-9-]{0,47})$/; got ${JSON.stringify(value)}`
    );
  }
}
