...
import { openPanelMemberThread, storePanelMemberConversation } from './reviewer-thread.js';
...
export function reducePanelRound({ roster: rawRoster, version: rawVersion, claude: rawClaude, members: rawMembers }) {
  ...
  for (let index = 0; index < rawMembers.length; index += 1) {
    ...
    if (entry.ok !== undefined && typeof entry.ok !== 'boolean') {
      throw unavailable(memberId, `ok must be a boolean when present; got ${JSON.stringify(entry.ok)}`);
    }
...
/**
 * Assemble the complete fixed-budget replay sent with every agy member turn (it carries the other
 * members' findings and seeds a replacement conversation if the member's conversation is lost).
 *
...
/**
 * Cheap activation probe mirroring resolveReviewPanel's `configured` rule without validating the
 * rest of the project config. A file that cannot be read or parsed carries no panel activation.
 * @param {string} repoRoot @param {Record<string, string | undefined>} [env]
 */
function reviewPanelActivated(repoRoot, env = process.env) {
  if (env.CODEX_PAIRED_REVIEW_PANEL_REVIEW !== undefined) return true;
  let parsed;
  try {
    parsed = JSON.parse(readFileSync(join(repoRoot, '.codex-paired', 'project.json'), 'utf8'));
  } catch {
    return false;
  }
  const block = parsed && typeof parsed === 'object' ? parsed.review_panel : undefined;
  return block !== null && typeof block === 'object' && Object.prototype.hasOwnProperty.call(block, 'review');
}
...
export function assertPanelRouteSupported({ repoRoot, split }) {
  ...
  // Unconfigured projects must behave exactly as before these routes knew about panels, so an
  // unrelated project-config problem must not start halting them here. Only when the review panel
  // is explicitly activated (file block or env override) is the full resolver consulted, and its
  // validation errors then surface as they should.
  if (!reviewPanelActivated(repoRoot)) return;
  const panel = resolveReviewPanel({ phase: 'review', repoRoot });
  if (panel.configured && panel.roster.length > 1) {
...
export async function reviewPanelMember(options, deps = {}) {
  ...
  // Only an accepted turn keeps its conversation; a rejected one leaves the stored id unchanged.
  await storePanelMemberConversation(/** @type {any} */ ({ ...options, member_id: memberId }), result);
  return {
    member_id: memberId,
    verdict: normalized,
...
export async function runPanelRound(input, deps = {}) {
  ...
  const memberResults = await Promise.all(roster.map((member) => dispatchMember({
    ...
    model: member.model,
    prompt,
    replay: typeof input?.replay === 'string' && input.replay.length > 0 ? input.replay : `Round: ${round}`,
