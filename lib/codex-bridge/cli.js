...
import {
  ...
  reducePanelRound,
  reviewPanelMember,
} from './review-panel-run.js';
import { writePreservationMarker } from './checkout-markers.js';

/** @typedef {Record<string, any>} CliArgs */
/** @typedef {(args: CliArgs) => unknown | Promise<unknown>} CommandHandler */
 ...
 */
/** @param {string} verb @param {string} flagName @param {unknown} value @returns {any} */
...
const subcommands = {
  'sidecar-init'({ specPath, feature, threadId, model, reasoning }) {
    let planning;
    if (!model || !reasoning) {
      planning = resolveModelsOrExit({ repoRoot: process.cwd() }).roles.planning;
    }
    const sc = initSidecar(specPath, {
      feature,
      codexSession: threadId,
      model: model || /** @type {import('./types.js').RoleConfig} */ (planning).model,
      reasoningEffort: reasoning || /** @type {import('./types.js').RoleConfig} */ (planning).effort,
    });
    process.stdout.write(JSON.stringify(sc, null, 2));
  },
  'model-role'({ role, repoRoot, format = 'json' }) {
    if (!MODEL_ROLES.includes(role)) {
      process.stderr.write(`model-role: unknown role ${JSON.stringify(role)}; expected one of: ${MODEL_ROLES.join(', ')}\n`);
      process.exit(2);
    }
    if (!['json', 'flags', 'mcp'].includes(format)) {
      process.stderr.write('model-role: --format must be one of: json, flags, mcp\n');
      process.exit(2);
    }
    const resolved = resolveModelsOrExit({ repoRoot: repoRoot || process.cwd() });
    const execInfo = cliExecArgs(role, resolved.roles);
    if (format === 'flags') {
      process.stdout.write(execInfo.args.join(' '));
      return;
    }
    if (format === 'mcp') {
      try {
        process.stdout.write(JSON.stringify(mcpCallConfig(role, resolved.roles)));
      } catch (/** @type {any} */ err) {
        process.stderr.write(`${err.detail || err.message}\n`);
        process.exit(2);
      }
      return;
    }
    process.stdout.write(JSON.stringify({
      role,
      cli: execInfo.cli,
      model: resolved.roles[role].model,
      effort: resolved.roles[role].effort,
      command: execInfo.command,
      args: execInfo.args,
      insertAfter: execInfo.insertAfter,
      sources: resolved.sources[role],
    }));
  },
  'review-panel'({ phase, repoRoot, format = 'json' }) {
    if (phase !== 'planning' && phase !== 'review') {
      process.stderr.write(`review-panel: --phase must be one of: planning, review; got ${JSON.stringify(phase)}\n`);
      process.exit(2);
    }
    if (format !== 'json' && format !== 'status') {
      process.stderr.write('review-panel: --format must be json or status\n');
      process.exit(2);
    }
    if (typeof repoRoot !== 'string' || repoRoot.length === 0) {
      process.stderr.write('review-panel: --repoRoot <repo> is required\n');
      process.exit(2);
    }
    try {
      const result = resolveReviewPanel({ phase, repoRoot });
      for (const warning of result.warnings) process.stderr.write(`warning: ${warning}\n`);
      // `status` adds whether the phase is explicitly configured: skills take the panel path only
      // when it is, and today's single-reviewer path otherwise (v0.19.0 roster activation rule).
      process.stdout.write(JSON.stringify(format === 'status'
        ? { configured: result.configured, roster: result.roster, warnings: result.warnings }
        : result.roster));
    } catch (error) {
  ...
  async 'review-panel-member'(args) {
    if (typeof args.role !== 'string' || typeof args.specPath !== 'string'
      || typeof args.repoRoot !== 'string' || typeof args['member-id'] !== 'string'
      || typeof args.model !== 'string' || typeof args.version !== 'string'
      || typeof args['prompt-file'] !== 'string') {
      process.stderr.write('review-panel-member: --role, --specPath, --repoRoot, --member-id, --model, --version, and --prompt-file are required\n');
      process.exit(2);
    }
    let roundPrompt;
    try {
      roundPrompt = readFileSync(args['prompt-file'], 'utf8');
    } catch (/** @type {any} */ error) {
      process.stderr.write(`review-panel-member: cannot read --prompt-file: ${error.message}\n`);
      process.exit(2);
    }
    if (roundPrompt.trim().length === 0) {
      process.stderr.write('review-panel-member: --prompt-file is empty\n');
      process.exit(2);
    }
    if (args.role !== 'planning' && args.role !== 'review') {
      process.stderr.write(`review-panel-member: --role must be "planning" or "review"; got ${JSON.stringify(args.role)}\n`);
      process.exit(2);
    }
    if (args['member-id'] !== `agy:${args.model}`) {
      process.stderr.write('review-panel-member: --member-id must equal agy:<model>\n');
      process.exit(2);
    }
    let timeout_ms;
    if (args['timeout-ms'] !== undefined) {
      ...
      if (!Number.isInteger(timeout_ms) || timeout_ms < 0) {
        process.stderr.write('review-panel-member: --timeout-ms must be a non-negative integer\n');
        process.exit(2);
      }
    }
    const replay = await readStdin();
    if (replay.length === 0) {
      process.stderr.write('review-panel-member: bounded replay is required on stdin\n');
      process.exit(2);
    }
    try {
      const result = await reviewPanelMember({
        role: args.role,
  ...
  async 'panel-preflight'({ phase, repoRoot }) {
    if (phase !== 'planning' && phase !== 'review') {
      process.stderr.write(`panel-preflight: --phase must be one of: planning, review; got ${JSON.stringify(phase)}\n`);
      process.exit(2);
    }
    if (typeof repoRoot !== 'string' || repoRoot.length === 0) {
      process.stderr.write('panel-preflight: --repoRoot <repo> is required\n');
      process.exit(2);
    }
    let roster;
    try {
      roster = resolveReviewPanel({ phase, repoRoot }).roster;
    } catch (/** @type {any} */ error) {
      process.stderr.write(`${error.code ?? 'error'}: ${error.detail ?? error.message}\n`);
      process.exit(2);
    }
    try {
      await assertPanelMembersAvailable({ repoRoot, roster });
    } catch (/** @type {any} */ error) {
      process.stderr.write(`${error.message}\n`);
      if (error instanceof PanelRuntimeError) process.exit(1);
      process.exit(2);
    }
    process.stdout.write(JSON.stringify({ available: roster.map((member) => member.member_id) }));
  },
  async 'panel-replay'() {
    ...
    try {
      ...
      process.stderr.write(`panel-replay: stdin must be valid JSON: ${error.message}\n`);
      process.exit(2);
    }
    try {
      process.stdout.write(composeMemberReplay(input ?? {}));
    } catch (/** @type {any} */ error) {
      process.stderr.write(`${error.message}\n`);
      if (error instanceof PanelRuntimeError) process.exit(1);
      process.exit(2);
  ...
  async 'panel-reduce'() {
    ...
    try {
      ...
      process.stderr.write(`panel-reduce: stdin must be valid JSON: ${error.message}\n`);
      process.exit(2);
    }
    try {
      process.stdout.write(JSON.stringify(reducePanelRound(input)));
    } catch (/** @type {any} */ error) {
      if (error instanceof PanelRuntimeError) {
        process.stderr.write(`${error.message}\n`);
        process.exit(1);
  ...
  'checkout-preserve'({ path, reason, run, repoRoot }) {
    for (const [name, value] of Object.entries({ path, reason, run, repoRoot })) {
      if (typeof value !== 'string' || value.length === 0) {
        process.stderr.write(`checkout-preserve: --${name} <value> is required\n`);
        process.exit(2);
      }
    }
    let resolvedRepo;
    try {
      resolvedRepo = realpathSync(resolve(/** @type {string} */ (repoRoot)));
    } catch {
      process.stderr.write(`checkout-preserve: --repoRoot must identify an existing repository\n`);
      process.exit(2);
    }
    let resolvedPath;
    try {
      resolvedPath = realpathSync(resolve(/** @type {string} */ (path)));
    } catch {
      process.stderr.write(`checkout-preserve: ${resolve(/** @type {string} */ (path))} is not a registered worktree\n`);
      process.exit(2);
    }
    ...
    try {
      ...
      registered = output.split('\n')
        .filter((line) => line.startsWith('worktree '))
        .some((line) => {
          try {
            return realpathSync(resolve(line.slice('worktree '.length).trim())) === resolvedPath;
          } catch {
            return false;
          }
        });
    ...
    if (!registered) {
      process.stderr.write(`checkout-preserve: ${resolvedPath} is not a registered worktree of ${resolvedRepo}\n`);
      process.exit(2);
    }
    writePreservationMarker(resolvedPath, {
      reason: /** @type {string} */ (reason),
      run_id: /** @type {string} */ (run),
    });
  },
  async 'worktree-reap'({ apply, repoRoot }) {
    if (typeof repoRoot !== 'string' || repoRoot.length === 0) {
      process.stderr.write('worktree-reap: --repoRoot <repo> is required\n');
      process.exit(2);
    }
    if (apply !== undefined && apply !== true) {
      process.stderr.write('worktree-reap: --apply does not take a value\n');
      process.exit(2);
    }
    let resolvedRepo;
    try {
      resolvedRepo = realpathSync(resolve(repoRoot));
      const probe = execFileSync('git', ['rev-parse', '--git-common-dir'], {
        cwd: resolvedRepo,
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'pipe'],
      });
      if (probe.trim().length === 0) throw new Error('empty git directory');
    } catch {
      process.stderr.write('worktree-reap: --repoRoot must identify an existing git repository\n');
      process.exit(2);
    }
    const { reap: reapCheckouts } = await import('./checkout-reaper.js');
    const result = reapCheckouts({
  ...
  'reviewer-thread-open': async (args) => {
    if (!args.role || !args.specPath || !args.repoRoot) {
      process.stderr.write('reviewer-thread-open: --role, --specPath, and --repoRoot are required (not implemented until slice 4)\n');
      process.exit(2);
    }
    if (args.role !== 'planning' && args.role !== 'review') {
      process.stderr.write(`reviewer-thread-open: --role must be "planning" or "review"; got ${JSON.stringify(args.role)}\n`);
      process.exit(2);
    }
    const prompt = await readStdin();
    try {
      const { openReviewerThread } = await import('./reviewer-thread.js');
      ...
      if (!result.ok) {
        process.stderr.write(`%s: reviewer turn failed (status ${result.status}, exit ${result.exit}): ${(result.warnings || []).join('; ')}\n`.replace('%s', 'reviewer-thread-open'));
        process.exit(1);
      }
    } catch (/** @type {any} */ err) {
  ...
  'reviewer-thread-reply': async (args) => {
    if (!args.role || !args.specPath || !args.repoRoot) {
      process.stderr.write('reviewer-thread-reply: --role, --specPath, and --repoRoot are required (not implemented until slice 4)\n');
      process.exit(2);
    }
    if (args.role !== 'planning' && args.role !== 'review') {
      process.stderr.write(`reviewer-thread-reply: --role must be "planning" or "review"; got ${JSON.stringify(args.role)}\n`);
      process.exit(2);
    }
    const prompt = await readStdin();
    try {
      const { continueReviewerThread } = await import('./reviewer-thread.js');
      ...
      if (!result.ok) {
        process.stderr.write(`%s: reviewer turn failed (status ${result.status}, exit ${result.exit}): ${(result.warnings || []).join('; ')}\n`.replace('%s', 'reviewer-thread-reply'));
        process.exit(1);
      }
    } catch (/** @type {any} */ err) {
      process.stderr.write(`${err.message}\n`);
  ...
  'model-roles'({ repoRoot }) {
    const resolved = resolveModelsOrExit({ repoRoot: repoRoot || process.cwd() });
    process.stdout.write(JSON.stringify({
      ...resolved,
      validated_cli_version: CODEX_CLI_VALIDATED_VERSION,
    }));
  },
  ...
  async 'sidecar-rotate-thread-id'({ specPath, role, oldThreadId, newThreadId, reason, phase, round, threadConfig }) {
    ...
    await setCodexThreadId(specPath, {
      ...
      threadConfig: threadConfig === undefined
        ? undefined
        : requireJsonArg('sidecar-rotate-thread-id', 'threadConfig', threadConfig),
    });
  },
  ...
  'sidecar-set-panel-roster'({ specPath, phase, roster }) {
    if (typeof specPath !== 'string' || specPath.length === 0) {
      process.stderr.write('sidecar-set-panel-roster: --specPath <path> is required\n');
      process.exit(2);
    }
    if (phase !== 'planning' && phase !== 'review') {
      process.stderr.write('sidecar-set-panel-roster: --phase must be planning or review\n');
      process.exit(2);
    }
    try {
      setPanelRoster(specPath, phase, requireJsonArg('sidecar-set-panel-roster', 'roster', roster));
    } catch (/** @type {any} */ error) {
      process.stderr.write(`${error.message}\n`);
      process.exit(1);
    }
  ...
  'app-state-next-plan-context'({ specPath }) {
    ...
    const unshipped = s.goals.filter((/** @type {CliArgs} */ g) => !g.audited_shipped);
    const shipped = s.goals.filter((/** @type {CliArgs} */ g) => g.audited_shipped);
    process.stdout.write(JSON.stringify({
      unshipped_goals: unshipped.map((/** @type {CliArgs} */ g) => ({ id: g.id, text: g.text })),
      shipped_goals: shipped.map((/** @type {CliArgs} */ g) => ({ id: g.id, text: g.text, by_plan: g.shipped_by_plan })),
      shipped_plans: s.plans.filter((/** @type {CliArgs} */ p) => p.shipped).map((/** @type {CliArgs} */ p) => ({ path: p.path, audited_goals: p.audited_goals })),
  ...
  'honest-reporting-mark-active'({ skill, spec, 'ttl-hours': ttlHours, cwd }) {
    ...
    /** @type {{skillName: string, specPath?: string, ttlHours?: number}} */
  ...
  async 'parse-skip-frontmatter'() {
    ...
    if (error) {
      process.stderr.write(JSON.stringify({ defect: error.code, detail: error.detail }));
      process.exit(2);
    } else {
      process.stdout.write(JSON.stringify(result));
      process.exit(0);
...
/** @param {string[]} rest @returns {CliArgs} */
...
function resolveModelsOrExit(options) {
  try {
    return resolveModelRoles(options);
  } catch (error) {
    if (error instanceof ModelsConfigError) {
      process.stderr.write(`${error.code}: ${error.detail}\n`);
      process.exit(2);
...
Promise.resolve(fn(parseArgs(rest))).catch((/** @type {any} */ e) => {
