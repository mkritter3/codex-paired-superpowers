...
import { hasAuditFor } from './sidecar.js';
...
function formatAuditGateMessage(missing, specPath, phase, round) {
  const lines = [
    ...
    '',
    `Missing audit evidence for side(s): ${missing.join(', ')}.`,
    '',
    ...
    '    "side": "<claude|codex>",',
    '    "commands": [',
    '      {"cmd": "<grep / find / git log command you ran>", "summary": "<one-line result>"},',
    '      ...',
    '    ],',
...
export function decideAuditGate(toolName, toolInput, markerInfo, deps = {}) {
  ...
  if (phase !== 'spec' && phase !== 'plan') return { exit: 0 };

  const hasAuditFn = deps.hasAuditFor || hasAuditFor;
  const missing = [];

  if (startsWithShip(claudeVerdict)) {
    let ok = false;
    try {
      ok = hasAuditFn(specPath, { phase, round, side: 'claude' });
    } catch {
      return { exit: 0 };
    }
    if (!ok) missing.push('claude');
  }
  if (startsWithShip(codexVerdict)) {
    let ok = false;
    try {
      ok = hasAuditFn(specPath, { phase, round, side: 'codex' });
    } catch {
      return { exit: 0 };
    }
    if (!ok) missing.push('codex');
  }
