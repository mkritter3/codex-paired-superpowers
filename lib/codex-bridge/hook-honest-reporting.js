...
function formatAuditGateMessage(missing, specPath, phase, round) {
  // `missing` is an array of { side, reason } (v0.13.0). Reduce to side names for the summary line.
  const sides = missing.map((m) => (typeof m === 'string' ? m : m.side));
  const lines = [
    `Honest-reporting audit gate (v0.10.1): refusing to log SHIP verdict for ${phase} round ${round}.`,
    '',
    `Missing audit evidence for side(s): ${sides.join(', ')}.`,
    '',
    'Both Claude and Codex must independently audit the artifact against the codebase',
    'before either side claims SHIP. The audit log makes that evidence replayable.',
    '',
    'Required action — for each side claiming SHIP, append an audit entry first:',
    '',
    "  printf '%s' '{",
    `    \"phase\": \"${phase}\",`,
    `    \"round\": ${round},`,
    '    "side": "<claude|codex>",',
    '    "commands": [',
    '      {"cmd": "<grep / find / git log command you ran>", "summary": "<one-line result>", "kind": "inspection"},',
    '      ...',
    '    ],',
    '    "verdict_basis": "<one-line: how the audit informed your SHIP>"',
    "  }' | node ${CLAUDE_PLUGIN_ROOT}/lib/codex-bridge/cli.js sidecar-append-audit \\",
    `    --specPath "${specPath}"`,
    '',
    "After both sides' audits are recorded, re-run the sidecar-append-round command.",
    '',
    'If you genuinely did not perform a codebase audit, do NOT fabricate one — emit REVISE',
    'instead and ask the other side to perform the audit in the next round.',
  ];
  return lines.join('\n');
}
