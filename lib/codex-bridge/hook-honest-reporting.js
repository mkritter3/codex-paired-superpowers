...
import { hasAuditFor, hasExecutedVerificationFor, requiresExecutedVerification } from './sidecar.js';
 ...
 * @returns {{exit: 0|2, message?: string}}
 */
export function decidePreToolUse(toolName, toolInput, recentTurns, markerInfo, deps = {}) {
  if (!markerInfo || !markerInfo.active) return { exit: 0 };
  if (toolName !== 'Bash') return { exit: 0 };

  // v0.10.1 audit gate runs first; the claim-vocab scan below still runs
  // for blocklisted release commands when the audit gate passes.
  const auditGate = decideAuditGate(toolName, toolInput, markerInfo, deps);
  if (auditGate.exit !== 0) return auditGate;
...
// v0.10.1 audit gate: when brainstorming or writing-plans is the active
// skill, a SHIP verdict cannot be logged via sidecar-append-round unless an
// audit entry already exists for the same (phase, round, side). This makes
// "verified against the codebase before claiming SHIP" a machine-enforced
// contract. Parallel to the VERIFIED/ASSUMED/UNTESTED claim-vocab gate.

const SIDECAR_APPEND_ROUND_RE = /\bsidecar-append-round\b/;

/**
 * Parse a sidecar-append-round Bash command into structured fields.
 * Returns null on any parse failure (fail-open is the rule).
 */
export function parseSidecarAppendRoundCommand(cmd) {
  if (typeof cmd !== 'string' || !SIDECAR_APPEND_ROUND_RE.test(cmd)) return null;

  const specMatch = /--specPath\s+(?:"([^"]+)"|'([^']+)'|(\S+))/.exec(cmd);
  if (!specMatch) return null;
  const specPath = specMatch[1] || specMatch[2] || specMatch[3];
  if (!specPath || specPath.length === 0) return null;

  let jsonText = null;
  const singleM = /--round\s+'((?:\\'|[^'])+)'/.exec(cmd);
  if (singleM) {
    jsonText = singleM[1].replace(/\\'/g, "'");
  } else {
    const doubleM = /--round\s+"((?:\\"|[^"])+)"/.exec(cmd);
    if (doubleM) jsonText = doubleM[1].replace(/\\"/g, '"');
  }
  if (!jsonText) return null;

  let round;
  try {
    round = JSON.parse(jsonText);
  } catch {
    return null;
  }
  ...
  return {
    ...
    phase: round.phase,
    round: round.round,
    claudeVerdict: typeof round.claude === 'string' ? round.claude : '',
    codexVerdict: typeof round.codex === 'string' ? round.codex : '',
  };
}

function startsWithShip(verdict) {
  return /^\s*SHIP(?![A-Z0-9_])/.test(verdict);
}

function formatAuditGateMessage(missing, specPath, phase, round) {
  // `missing` is an array of { side, reason } (v0.13.0), reason ∈ 'no-audit' | 'no-verification'.
  // Tolerate legacy string entries for safety.
  const norm = missing.map((m) => (typeof m === 'string' ? { side: m, reason: 'no-audit' } : m));
  const sides = norm.map((m) => m.side);
  const reasonText = {
    'no-audit': 'no audit entry recorded — append one with sidecar-append-audit (include "kind")',
    'no-verification': 'audit exists but has no successful kind:"verification" command — run the project\'s test/build and record it with "exit_code": 0 before SHIP',
  };
  const lines = [
    `Audit evidence required before logging SHIP for ${phase} round ${round}.`,
    '',
    // Keep an explicit side list so the missing side(s) are unambiguous at a glance.
    `Missing audit evidence for side(s): ${sides.join(', ')}.`,
    '',
    'This is an expected, actionable step — not a crash. For each side below:',
    ...norm.map((m) => `  - ${m.side}: ${reasonText[m.reason] || reasonText['no-audit']}`),
    '',
    'Record the evidence, then log the round. Prefer the atomic happy-path command so audits and the',
    'round are written together (the round can never be logged before its audits):',
    '',
    "  printf '%s' '{",
    '    "audits": [',
    `      {"phase": "${phase}", "round": ${round}, "side": "claude",`,
    '       "commands": [{"cmd": "<grep/find/git log OR npm test>", "summary": "<result>", "kind": "inspection"}],',
    '       "verdict_basis": "<one-line>"},',
    `      {"phase": "${phase}", "round": ${round}, "side": "codex", "commands": [{"cmd": "...", "summary": "...", "kind": "inspection"}], "verdict_basis": "..."}`,
    '    ],',
    `    "round": {"phase": "${phase}", "round": ${round}, "claude": "SHIP", "codex": "SHIP"}`,
    "  }' | node ${CLAUDE_PLUGIN_ROOT}/lib/codex-bridge/cli.js sidecar-append-round-with-audits \\",
    `    --specPath "${specPath}"`,
    '',
    'For a code-bearing phase (implement/review-slice/live-verification/post-merge-review), at least one',
    'command must be {"kind": "verification", "exit_code": 0} — an executed test/build, not just inspection.',
    '',
    '(You can still append a single side with sidecar-append-audit if recovering manually.)',
    '',
    'If you genuinely did not verify, do NOT fabricate evidence — emit REVISE and ask the other side to',
    'perform the audit in the next round.',
  ];
  return lines.join('\n');
}

/**
 * Pure decision for the audit gate. Exported for tests; called from
 * decidePreToolUse() above. Fail-open on any parse / arg failure.
 */
export function decideAuditGate(toolName, toolInput, markerInfo, deps = {}) {
  if (!markerInfo || !markerInfo.active) return { exit: 0 };
  if (toolName !== 'Bash') return { exit: 0 };

  const cmd = toolInput && typeof toolInput.command === 'string' ? toolInput.command : '';
  const parsed = parseSidecarAppendRoundCommand(cmd);
  if (!parsed) return { exit: 0 };

  const { specPath, phase, round, claudeVerdict, codexVerdict } = parsed;
  if (typeof phase !== 'string' || !Number.isInteger(round)) return { exit: 0 };
  ...
  // v0.13.0: design phases (spec, plan, plan-slice:<id>) use the inspection-audit gate;
  // code-bearing phases (implement/review-slice/live-verification/post-merge-review:<id>) require
  // an executed verification command. Any other phase is out of scope for this gate.
  const isDesignPhase = phase === 'spec' || phase === 'plan' || /^plan-slice:/.test(phase);
  const isCodeBearing = requiresVerifFn(phase);
  if (!isDesignPhase && !isCodeBearing) return { exit: 0 };

  // Each missing entry is { side, reason } where reason ∈ 'no-audit' | 'no-verification'.
  // Slice 2 keeps the message generic; Slice 4 renders the per-side reason precisely.
  const missing = [];

  function check(side, verdict) {
    if (!startsWithShip(verdict)) return;
    let ok = false;
    try {
      ok = isCodeBearing
        ? hasVerifFn(specPath, { phase, round, side })
        : hasAuditFn(specPath, { phase, round, side });
    } catch {
      missing.failOpen = true;
      return;
    }
    if (!ok) missing.push({ side, reason: isCodeBearing ? 'no-verification' : 'no-audit' });
  }

  check('claude', claudeVerdict);
  if (missing.failOpen) return { exit: 0 };
  check('codex', codexVerdict);
  if (missing.failOpen) return { exit: 0 };

  if (missing.length === 0) return { exit: 0 };
  return { exit: 2, message: formatAuditGateMessage(missing, specPath, phase, round) };
}
