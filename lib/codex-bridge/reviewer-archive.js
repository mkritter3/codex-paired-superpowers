...
// Kept separate from the legacy exported registry because expert-archive.js exposes
// that Set as a one-window compatibility API with an exact-content contract.
const V016_HALT_REASONS_PRESERVE = new Set([
...
export async function archive(identity, haltReason, deps = {}) {
  ...
  if (HALT_REASONS_PRESERVE.has(haltReason) || V016_HALT_REASONS_PRESERVE.has(haltReason)) {
    return {
      expert_id: identity.id,
