 ...
 *
 * `expert-*` ids map to `expert-<suffix>.md` (1:1 with the filename).
 * `paired-reviewer` maps to `system-rubric.md` because the rubric IS the
 * paired-reviewer's base prompt; the explicit indirection here keeps the
 * disk layout decoupled from the role-id taxonomy.
 *
 * @param {string} roleId
 * @returns {string} file basename (e.g. "expert-architecture.md")
 */
function roleIdToFilename(roleId) {
  if (roleId === 'paired-reviewer') return 'system-rubric.md';
  if (typeof roleId === 'string' && roleId.startsWith('expert-')) {
    return `${roleId}.md`;
  }
  throw new RolePromptError(
    `unknown role id "${roleId}" — expected "paired-reviewer" or "expert-*"`,
    { roleId }
  );
}
...
export function loadRolePrompt(roleId, opts = {}) {
  const filename = roleIdToFilename(roleId);
  ...
  const { frontmatter, body } = parseFrontmatter(text, roleId);
  ...
  if (frontmatter.role_id !== roleId) {
    throw new RolePromptError(
      `role prompt at ${filename} declares role_id="${frontmatter.role_id}" ` +
        `but was loaded as "${roleId}" — fix the frontmatter or the caller`,
      { roleId }
    );
  }
  return {
