...
test('isCacheFresh returns true when within TTL and versions match', () => {
  ...
  // 30 minutes later, well under 1h TTL.
  assert.equal(isCacheFresh(payload, cachedAtMs + 30 * 60 * 1000), true);
});
...
test('isCacheFresh returns false when TTL exceeded', () => {
  ...
  // 61 minutes later → past 1h default.
  assert.equal(isCacheFresh(payload, cachedAtMs + 61 * 60 * 1000), false);
  // And a custom TTL also works:
  assert.equal(isCacheFresh(payload, cachedAtMs + 10_000, 5_000), false);
});
