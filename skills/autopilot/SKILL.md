...
`node scripts/cli-surface.mjs --digest --write` **from the slice worktree root** (the script and the
document it rewrites are the worktree's own; the script defaults `--root` to the repository it lives
in, so invoking the installed plugin's copy would refresh the plugin checkout instead — if you must
call another copy, pass `--root <slice-worktree>`), and commit the `docs/public-api.md` diff onto the
slice branch so the reviewed SHA carries it (`npm test` fails on a stale digest, and Phase C requires
green verification). `--write` touches only those three keys; Phase D keeps the semantic review of
